...
- **Type**: cslib
- **Created**: 2026-07-27
- **Last updated**: 2026-07-27 (cycle-23 close — Phase 5 remains the sole open workstream at 304
  sites done, 46 local-only blanket suppressions across 32 files remaining (down from 72/49 at
  cycle 22's close; 17 files cleared this cycle) — figure re-verified live at this update; see
  "Upstream-exposure scope" for the rescope this denominator is measured against. Phase 7 remains
  CLOSED per the cycle-22 user-decision pass: the user authorized the local `NOTATION.md` edit as
  an explicit, recorded exception to the upstream-exposure carve-out, and directed that the false
  "stale" premise behind the 5 `NOTE:`-block deletions be struck rather than executed.)
...
Twenty-fourth resume (cycle 23 closure — processed via `/implement`, resuming the
paused `/orchestrate` invocation per its continuation_context). Status as of this pass: **Phases
1, 2, 3, 4, 6, 8 all COMPLETED (Phase 1's regression was re-closed in cycle 13; Phase 3 was
re-closed in cycle 12 after its own reopen; Phase 4's post-close `lake shake` regression was
re-closed in cycle 16); Phase 7 is CLOSED (both blockers resolved by user decision, recorded
above, not re-investigated this cycle). Only Phase 5 (suppression audit) remains, PARTIAL at 304
sites done (85 files fully processed), 46 blanket suppressions across 32 local-only files
remaining in scope (down from 72/49 at cycle 22's close). This cycle processed 17 files, all
1-2→0 clean, each individually verified by scoped rebuild of the file plus every direct
downstream importer (two heavily-imported core files — `Bimodal/ProofSystem/Derivation.lean` and,
transitively via its own downstream fan-out, `Bundle/CanonicalFrame.lean` — were instead verified
with a full `lake build`/targeted multi-target rebuild rather than an exhaustively enumerated
importer list) before its own commit (17 individual commits):
`Temporal/Metalogic/Chronicle/ChronicleToCountermodel.lean` (137 lines; 1 bare-`simp` flexible
site replaced with the linter-suggested `simp only [ne_eq, Subtype.mk.injEq]`),
`Bimodal/Metalogic/Algebraic/ParametricCompleteness.lean` (143 lines; 1 longLine wrap),
`Bimodal/Metalogic/Decidability/Correctness.lean` (147 lines; 1 `unusedSectionVars` +
`unusedDecidableInType` pair on `decide_sound`, both resolved together via
`omit [DecidableEq Atom] [Hashable Atom] in`, matching the existing pattern already used in
`SignedFormula.lean`/`BDriver.lean`), `Bimodal/Metalogic/BXCanonical/OrderedSeedConsistency.lean`
(149 lines; 3 longLine wraps), `Bimodal/Metalogic/Algebraic/ParametricCanonical.lean` (156 lines;
1 longLine doc-comment wrap), `Bimodal/Metalogic/Algebraic/InteriorOperators.lean` (176 lines; 5
`show`-tactic sites the linter flagged as goal-changing, each replaced with the
linter-recommended `change`), `Bimodal/Metalogic/Bundle/ModalSaturation.lean` (207 lines; 18
longLine wraps — the largest single-file wrap count this cycle),
`Bimodal/Metalogic/Bundle/CanonicalFrame.lean` (267 lines; 12 longLine wraps across four
`canonical_forward_F`/`backward_P`/`forward_U`/`backward_S` signatures plus two `have`-statement
sites), `Bimodal/Metalogic/Bundle/TemporalContent.lean` (173 lines — newly surfaced at fs=2 this
cycle, not on cycle 22's tracked count-2 list; 4 longLine wraps across `have`-statement type
annotations), `Temporal/Metalogic/Chronicle/CanonicalChain.lean` (76 lines; 1
`style.emptyLine` blanket removed, zero warnings surfaced — unnecessary),
`Temporal/ProofSystem/Derivation.lean` (98 lines; same, zero warnings surfaced),
`Bimodal/Metalogic/BXCanonical/Quasimodel/SubformulaClosure.lean` (99 lines; same, zero warnings,
no downstream importers), `Bimodal/FrameConditions/Soundness.lean` (117 lines; same, zero
warnings), `Bimodal/FrameConditions/Validity.lean` (118 lines; same, zero warnings),
`Temporal/Metalogic/Chronicle/OrderedSeedConsistency.lean` (135 lines; same, zero warnings),
`Bimodal/Metalogic/Separation/IntHelpers.lean` (159 lines; same, zero warnings),
`Bimodal/ProofSystem/Derivation.lean` (168 lines; same, zero warnings — this is the shared
`DerivationTree` base type for Bimodal, imported (directly or transitively) by most of the
Bimodal tree, so verified via a full `lake build` (3309/3309, only the 5 documented baseline
sorry warnings) rather than an enumerated importer list).
**The count-2 tier now has 9 files remaining** (down from 19 at cycle 22's close; the 9 smallest
were all cleared this cycle — see the file list above). The count-6 tier remains at exactly 1
file (`Interface.lean`, 3048 lines, still deliberately deferred). **Two files inspected this
cycle turned out to be permanent, already-justified exceptions, not further work**:
`Computability/Automata/DA/Conversions.lean` (117 lines, `linter.privateModule`) carries an
inline comment explaining the file consists exclusively of `proof_wanted` stubs with no
declaration to attach the suppression to, so narrowing is impossible without changing
mathematical content; `Bimodal/Metalogic/BXCanonical/Filtration/DefectChain.lean` (120 lines,
`linter.style.openClassical`) carries an inline comment explaining the file-wide `open Classical`
(not per-declaration) is required for `Finset.filter`'s `Classical.propDecidable` instance
propagation, and a `... in` scoping would silently break it. Both join `QLemma.lean` as
documented, permanent 1-line-suppression end states — do not attempt to reduce either further;
treat their presence on the worklist below as informational, not actionable.

**Live re-derived survey of the full remaining worklist, taken at this cycle's close** (fs =
blanket-suppression count, lc = line count, sorted fs desc then lc asc; re-verify both live
before trusting, since counts only ever go down and this list will be stale by the next cycle):
...
Count-2 tier (9 files, smaller-first is the recommended order — NEXT TARGET is
`ChronicleInterface.lean` or `TruthLemma.lean`, both 294 lines):
```
2  294 Cslib/Foundations/Logic/Metalogic/Chronicle/ChronicleInterface.lean
2  294 Cslib/Logics/Temporal/Metalogic/Chronicle/TruthLemma.lean
2  298 Cslib/Logics/Bimodal/Metalogic/Bundle/SuccRelation.lean
2  301 Cslib/Logics/Bimodal/Metalogic/Decidability/FMP/Filtration.lean
2  363 Cslib/Foundations/Logic/Metalogic/Chronicle/Types.lean
2  390 Cslib/Logics/Bimodal/ProofSystem/Substitution.lean
2  398 Cslib/Logics/Bimodal/Metalogic/Bundle/TemporalCoherence.lean
2  605 Cslib/Logics/Bimodal/Metalogic/Bundle/WitnessSeed.lean
2 1104 Cslib/Logics/Bimodal/Metalogic/Soundness/DenseValidity.lean
```
(Note, carried forward from cycle 22: `ChronicleInterface.lean` and `Types.lean` both live in
`Foundations/Logic/Metalogic/Chronicle/` alongside `SinceSeedConsistency.lean`, closed cycle 22
— check whether either reuses that file's bare-`simp [...] at h` flexible pattern before
re-deriving an approach from scratch. `TruthLemma.lean` is in `Temporal/Metalogic/Chronicle/`,
alongside `ChronicleToCountermodel.lean`, `CanonicalChain.lean`, and
`OrderedSeedConsistency.lean`, all closed this cycle or cycle 21 — same caveat, and also check
whether it reuses `ChronicleToCountermodel.lean`'s bare-`simp` → linter-suggested-`simp only`
pattern from this cycle. `SuccRelation.lean`, `TemporalCoherence.lean`, and `WitnessSeed.lean`
all live in `Bimodal/Metalogic/Bundle/` alongside `CanonicalFrame.lean`, `ModalSaturation.lean`,
and `TemporalContent.lean`, all closed this cycle — check for shared longLine wrap patterns
before re-deriving from scratch, since three consecutive Bundle-directory files this cycle were
pure longLine-wrap jobs with no other category.)

Count-1 tier (22 files, smaller-first; `Conversions.lean` and `DefectChain.lean` from cycle 22's
list are REMOVED from this tier — see the permanent-exception note above):
```
1  172 Cslib/Logics/Bimodal/Metalogic/Separation/Distributivity.lean
1  178 Cslib/Logics/Bimodal/Metalogic/Separation/NegationEquiv.lean
1  187 Cslib/Logics/Bimodal/Metalogic/Decidability/FMP/FMP.lean
1  223 Cslib/Logics/Bimodal/Theorems/Perpetuity/Bridge.lean
1  230 Cslib/Logics/Temporal/Metalogic/TemporalContent.lean
1  240 Cslib/Logics/Bimodal/Metalogic/BXCanonical/Chronicle/CounterexampleElimination/Elimination.lean
1  261 Cslib/Logics/Temporal/Metalogic/MetricSoundness.lean
1  275 Cslib/Logics/Bimodal/Metalogic/Separation/FormulaOps.lean
1  295 Cslib/Logics/Bimodal/Metalogic/Decidability/FMP/ClosureMCS.lean
...
1  435 Cslib/Logics/Bimodal/Metalogic/Separation/DedekindZ/QLemma.lean
1  538 Cslib/Logics/Temporal/Metalogic/MetricCompleteness.lean
1  655 Cslib/Logics/Bimodal/Metalogic/Separation/Defs.lean
1  871 Cslib/Logics/Bimodal/Metalogic/Separation/Eliminations.lean
1 1208 Cslib/Logics/Bimodal/Metalogic/Decidability/Tableau.lean
```
(`QLemma.lean` is documented in earlier cycles as correctly bottoming out at 1 remaining blanket
line for a persistent non-`in` `open Classical` — verify this is still that same, correct,
permanent end state before attempting further reduction on it; do not treat "1 remaining" alone
as a signal that it is incomplete. `Eliminations.lean` carries the same documented
`style.openClassical` end-state pattern.)

Total: 46 blanket suppressions across 32 local-only files (1 count-6 + 9 count-2 + 22 count-1
tier files — note the count-1 list above shows 20 rows, not 22; `Conversions.lean` and
`DefectChain.lean`, both permanent exceptions, are excluded from the actionable list but still
counted in the 32/46 totals and in `check-lint-suppressions.sh`'s repo-wide ratchet). At this
cycle's rate (26 sites/17 files, mostly mechanical single- and few-site fixes), and given the
count-1 tier's actionable files average almost exactly 1 site each, clearing the remainder is
estimated at **roughly 2-4 more cycles**, faster than the prior 4-6-cycle estimate now that the
count-1 tier's two permanent exceptions are identified and the count-2 tier is down to 9 files.

Re-derive the live worst-offender list with (filter to `fs > 0`, then gate each candidate through
the upstream check before treating it as in-scope):
```bash
...
```
Next target as of cycle 23's close: `Foundations/Logic/Metalogic/Chronicle/ChronicleInterface.lean`
or `Logics/Temporal/Metalogic/Chronicle/TruthLemma.lean` (both 294 lines, tied for smallest in
the count-2 tier), per smaller-files-first.

**New findings this cycle**: (1) not every file on a prior cycle's worklist is actionable —
`Conversions.lean` and `DefectChain.lean` both carry pre-existing inline comments justifying a
permanent blanket suppression (a `proof_wanted`-only file with no declaration to attach a
narrower suppression to; a file-wide `open Classical` whose declaration-scoped equivalent would
silently break instance propagation), so always read the file's existing comments before
assuming a worklist entry needs work — this is the same pattern as `QLemma.lean`/
`Eliminations.lean`'s permanent `style.openClassical` end state, just for two more categories
(`privateModule`, and a second `openClassical` instance); (2) a bare `simp` (not `simp [X]`) that
the `linter.flexible` warning suggests replacing with a specific `simp only [...]` is usually
directly usable verbatim — `ChronicleToCountermodel.lean`'s single site took the suggested
`simp only [ne_eq, Subtype.mk.injEq]` with no adjustment needed and rebuilt clean immediately;
(3) `linter.style.show` (a `show` tactic the linter says "changed the goal") is mechanically
fixable by replacing `show` with `change` — semantically identical (`change` is the tactic
`show` is discouraged from doing implicitly), confirmed via rebuild across all 5 sites in
`InteriorOperators.lean`; this is a new category not seen in cycles 1-22's findings; (4) three
consecutive files in `Bimodal/Metalogic/Bundle/` this cycle (`ModalSaturation.lean`,
`CanonicalFrame.lean`, `TemporalContent.lean`) were pure `emptyLine`+`longLine` jobs with no
`flexible`/`dupNamespace`/`unusedSectionVars` complexity at all, and `ModalSaturation.lean` alone
had 18 longLine sites — the Bundle directory appears to skew toward pure-longLine files, worth
checking first for the three remaining Bundle-directory count-2 files (`SuccRelation.lean`,
`TemporalCoherence.lean`, `WitnessSeed.lean`); (5) a file not on a prior cycle's tracked worklist
can still surface at the live re-derivation (`TemporalContent.lean` in `Bundle/`, fs=2, was not
in cycle 22's 19-file count-2 list) — always trust the live `grep`/upstream-gate re-derivation
over a prior cycle's static snapshot, per the standing instruction, and treat any such
discrepancy as itself worth a one-line note rather than silently absorbing it; (6) for a
heavily-imported core file (many dozens of transitive importers), enumerating every downstream
importer individually is not practical — a full `lake build` (confirming exact baseline sorry
count, zero new warnings) is an acceptable, faster substitute verification method for exactly
this case (used for `Bimodal/ProofSystem/Derivation.lean` this cycle).

**Cycle 21-22 findings, still relevant** (see the plan's git history for full cycle-20/21/22 body
text if a specific mechanism needs re-deriving; summarized here): declaration-scoped
`dupNamespace` narrowing for intentional structure/namespace-name collisions (the `Chronicle`
pattern); `longLine` wraps on `have h : DerivationTree ... := ...` are always mechanically safe;
`replace_all` for verbatim-recurring `simp only [...]` blanket-replacement candidates only after
confirming every site has the identical fully-reduced argument list; bare `simp [X]` pulling the
full default simp set favors declaration-scoped `linter.flexible false in` over manual
reconstruction; the `unusedDecidableInType` vs `unusedSectionVars` distinction and when `omit
[...] in` is the correct combined fix (an *unused* instance in an *unused* section-var position,
confirmed on `Correctness.lean`'s `decide_sound` this cycle); `lake lint`'s `unusedArguments`
findings are a distinct, out-of-scope linter, unchanged at 147 errors.

**Do not** re-derive the sorry census with a naive grep. Use the method in "Measurement notes"
(also implemented in `scripts/pre-pr-check.sh`). The `--wfail --iofail` build reconfirmed exactly
5 baseline *sorry* warnings at cycle 8, 12, 13, 14, 22, and 23's (this cycle's) end.
...
1. Confirm the baseline (4 commands, ~5 min) — **expect exactly 5 warnings, all documented
   sorries, lake shake reporting only the 12 upstream-shared files, and the ratchet reporting 60
   (baseline ceiling 60), as of cycle 23**:
   ```bash
   lake build --wfail --iofail   # expect exit 1; exactly 5 baseline sorry warnings
   ...
   bash scripts/check-lint-suppressions.sh   # expect exit 0, "60 (baseline ceiling 60)"
   ```
   If any of these differ from the above, something else landed — reconcile before proceeding.
...
3. **Only Phase 5 remains.** 85 files are fully processed cumulative (see Phase 5's cycle
   sub-entries, cycles 1 through 23, for the complete per-file list) — do not revisit any of
   them. Re-derive the live worst-offender list with the command above, then **filter it through
   the local-only gate** before picking a target, and **skip `Conversions.lean` and
   `DefectChain.lean`** (permanent exceptions, see above) if they appear. Start with the count-2
   tier, smaller-first: `ChronicleInterface.lean` or `TruthLemma.lean` (both 294 lines) next,
   using the method Phase 5's section documents: remove all of a file's suppressions, rebuild,
   categorize what surfaces, fix the mechanical categories, narrow the rest to
   declaration/usage-site scope.
4. **Before starting, read this cycle's six new findings above** (permanent-exception detection,
   the `simp`→suggested-`simp only` pattern, `show`→`change` for `linter.style.show`, the Bundle
   directory's pure-longLine skew, live-worklist files not on a prior static snapshot, and the
   full-`lake build` verification substitute for heavily-imported core files) plus the carried
   cycle 21-22 findings summarized above.
5. No items require a user decision to make further Phase 5 progress. Phase 7's two former
   blockers are CLOSED (see the top of this RESUME HERE section) and are not Phase 5 concerns.
...
### Phase 5: Suppression audit [PARTIAL — 304 sites done; 46 blanket suppressions across 32 local-only files remain in scope]
...
**Done (cycle 16)**: processed the smallest remaining count-5 file (per the cycle-15 addendum's
smallest-first ordering), committed after a clean scoped rebuild plus downstream-importer
rebuilds — `Bimodal/Metalogic/BXCanonical/Chronicle/PointInsertion/XuGuard.lean` (5→0, 1146
lines, commit `c0a11b8d`): `style.emptyLine`/`style.setOption` vestigial; `unusedSimpArgs` (1
site, `Formula.neg` unused in a `simp only` call) fixed mechanically; `style.longLine` (~59
sites, across the file's two structurally mirrored `lemma_2_7`/`lemma_2_8` proof blocks) fixed by
rewrapping; `flexible` (4 declarations: `l27_a_event_list_mem`, `l27_collect_guards_mem_of_B`,
`l27_guard_snce_xi_val`, `l27_collect_guards_mem_of_snce_xi`) narrowed to declaration-scoped
`set_option` lines — the same `l27`-family declaration names and fix pattern already seen in
`Splitting.lean` (cycle 15), confirming this file shares that helper-family structure.
Suppression-audit progress: 213 → 214 sites audited cumulative (34 files fully processed
cumulative). Repo-wide blanket suppression lines (ratchet-tracked): 228 → 223 (5 fewer;
re-baselined in the same commit). This cycle's dispatch was scope-split with item 1 (Phase 4's
...
**Cycle 18 (this dispatch) — 6 files processed, all committed individually after a clean scoped
rebuild plus downstream-importer rebuilds.** Worked the count-4 tier smaller-first per the
delegation instruction, then continued into it entirely: `Bimodal/Metalogic/Algebraic/
BooleanStructure.lean` (342 lines, 4→0, commit `638f6e70`), `Bimodal/Metalogic/
ConservativeExtension/ExtFormula.lean` (374 lines, 4→0, commit `dd479966`),
`Bimodal/Metalogic/Separation/DedekindZ/QLemma.lean` (436 lines, 4→1, commit `b69a2f27` — the 1
remaining line is a permanent, correct non-`in` `style.openClassical` suppression for the file's
persistent `open Classical`, matching the cycle-17 `Eliminations.lean` precedent, not residual
work), `Bimodal/Metalogic/BXCanonical/Frame.lean` (472 lines, 4→0, commit `449d0d63`),
`Foundations/Logic/Metalogic/Chronicle/RRelation.lean` (614 lines, 4→0, commit `043e4947`),
`Bimodal/Metalogic/BXCanonical/Chronicle/RRelation.lean` (780 lines, 4→0, commit `d666dbce`).
This clears the count-4 tier down to its single largest file (`HierarchyDefs.lean`, 1001 lines).
Suppression-audit progress: 222 → 246 sites audited cumulative (48 files fully processed
cumulative). Ratchet: 188 → 165 blanket suppressions repo-wide. Live per-file upstream/local-only
split (`git cat-file -e upstream/main:<path>` for every file currently carrying a blanket
suppression, filtered to `fs > 0` — an early pass over this cycle's re-derivation double-counted
41 files that contain some `set_option linter.*` line but zero *blanket* ones, which would have
under-reported the upstream-carved-out total; corrected by filtering to blanket-count > 0 before
the upstream gate) found 165 total = **151 local-only in-scope (73 files) + 14
upstream-carved-out (14 files, unchanged)**.

New category encountered this cycle: `linter.flexible` warnings where the "Try this:
simp only [...]" suggestion in the build log is **not directly usable as a literal
copy-paste replacement** — verified by rebuild failure (`simp made no progress` /
`Invalid ⟨...⟩ notation`) when applied verbatim in `ExtFormula.lean` and
`RRelation.lean` (both files). Root cause: the linter's info trace does not always include the
`embedFormula`/`ExtFormula.atoms`-style def-unfold lemmas that the original blanket `simp
[embedFormula, ...]` implicitly relied on (Lean's automatic iota/delta reduction during full
`simp` masks this dependency; `simp only [<the one suggested lemma>]` alone does not always
carry it). **Fix method going forward**: derive the simp-only set by reasoning about what the
flagged tactic's hypothesis/goal actually needs (the same def-unfold names used in the original
blanket call, plus the specific structural lemma — `injEq`, `Finset.mem_union`,
`decide_eq_true_eq`, etc. — that the linter names), then verify by rebuild; treat the "Try this"
line as a *hint*, not a ground truth, since it can quietly omit lemmas needed for the
tactic-after-`simp` to still type-check. This is now the 5th documented `linter.flexible`
sub-lesson (after the cycle-6 `simp?`-via-`lean_multi_attempt` guidance, cycle-9/17
`openClassical`, and cycle-14/15/16 declaration-mapping/duplicate-match/mirrored-block lessons).

Full CSLib CI pipeline run once at cycle end: `lake build --wfail --iofail` (exactly 5 baseline
sorry warnings — `FrameSoundness.lean:1252`, `Intuitionistic/Scheme.lean:570,2583`,
`Intuitionistic/Completeness.lean:124`, `Minimal/Completeness.lean:118` — zero new anywhere),
`lake exe checkInitImports` (clean, no output), `lake exe lint-style` (clean, no output), `lake
shake --add-public --keep-implied --keep-prefix` (exactly the 12 upstream-shared files flagged,
zero local-only findings), `lake exe mk_all --module` ("No update necessary"), `lake test` (exit
0, same 5 baseline sorry warnings plus the same one pre-existing unrelated
`backward.privateInPublic` warning in `CslibTests/FreeMonad.lean`). True sorry census
(comment-stripped, `warn.sorry`-excluded, per "Measurement notes", re-derived live rather than
trusted from the prior cycle's figure): 28, unchanged. Vacuous-def grep unchanged at the single
pre-existing false positive (`Computability/URM/Basic.lean:92`); axiom count unchanged at 26.
`git show` on each of this cycle's 6 commits individually confirmed zero `sorry` lines touched
(each diff checked, not just grepped in aggregate).

**Resume point (cycle 18 close)**: the 6 files above plus all files from cycles 1, 5, 6, 7, 8, 9,
10, 11, 12, 14, 15, 16, 17 are done — do not revisit (48 files total). Re-verify the worst-offender
list before starting the next cycle, since resolutions only ever remove entries, and re-gate every
candidate through `git cat-file -e upstream/main:<path>` (must FAIL) per the upstream-exposure
rescope, **filtering to files with a positive blanket count before the upstream gate** (the
cycle-18 correction above):
```bash
grep -rln "set_option linter\." Cslib/ | while read f; do
  fs=$(grep "set_option linter\." "$f" | grep -vc " in$")
  echo "$fs $f"
...
```
As of cycle 18's end (165 total remaining blanket suppression lines, **151 local-only in-scope
across 73 files**), the count-6 tier is still 5 files, unchanged from cycle 17 (none touched this
cycle): `Temporal/Metalogic/Chronicle/CounterexampleElimination/{RecursiveWalks (1125 lines),
MainElimination (1685 lines)}.lean`, `Bimodal/Metalogic/Soundness/FrameClassVariants.lean` (931
lines), `Bimodal/Metalogic/BXCanonical/Chronicle/CounterexampleElimination/Interface.lean` (3048
lines — do not pick this one first), `Bimodal/Metalogic/BXCanonical/Chronicle/
ChronicleToCountermodelBasic.lean` (1208 lines). **The count-4 tier is down to its final file**:
`Separation/Hierarchy/HierarchyDefs.lean` (1001 lines, confirmed local-only) — pick this next
per the established smaller-files-first-within-tier method (it is smaller than every count-6
file, so still ahead of that tier despite its own four-figure line count). Below it, a
count-3/count-2/count-1 tail of roughly 68 more files remains (spot-checked in earlier cycles,
not yet individually processed this cycle).

Apply the same method: remove all of a file's suppressions, rebuild, categorize what surfaces
into vestigial / mechanically-fixable / needs-real-proof-work, fix what's mechanical, narrow the
rest to declaration-scope, re-baseline the ratchet in the same commit — carrying forward every
safety lesson from cycles 6-18 (see the cycle-11/12 addenda above for the full list through cycle
12, the cycle-14 declaration-mapping-verification lesson, the cycle-15 Edit-tool
false-duplicate-match lesson, the cycle-16 mirrored-proof-block `replace_all` lesson, the
cycle-17 `openClassical`/`unusedArguments` findings, and this cycle's two findings: (1) the
`linter.flexible` "Try this" trace can omit def-unfold lemmas the original blanket `simp`
implicitly relied on — verify every suggested `simp only` replacement by rebuild, do not paste
it blind; (2) when re-deriving the worst-offender list, filter to `fs > 0` (files actually
carrying a *blanket* suppression) before running the upstream gate, or the upstream-carved-out
file count will be inflated by files that only contain declaration-scoped `... in` lines).

**Effort estimate for future cycles**: this cycle cleared 24 sites (6 files, 23 fully resolved +
1 permanent-end-state) in one dispatch — in line with the ~20-30 sites/cycle blended historical
rate, though slower per-file than cycle 17's 35-sites/8-files pace because two of this cycle's
files (`RRelation.lean` ×2) needed non-trivial `linter.flexible` narrowing rather than purely
mechanical `show`→`change`/longLine wraps. At the current blended rate, clearing the remaining
**151 local-only sites** is roughly **5-8 more cycles**.

**Cycle 19 (this dispatch) — 2 files processed, both committed individually after a clean
scoped rebuild plus downstream-importer rebuilds.** Per the resume instruction, worked
`Separation/Hierarchy/HierarchyDefs.lean` first (1001 lines, the count-4 tier's final file,
4→0, commit `ff84e0f2`), then moved into the count-6 tier smaller-first and processed
`Soundness/FrameClassVariants.lean` (931 lines, 6→0, commit `8258d8a3`). This clears the count-4
tier entirely and reduces the count-6 tier from 5 files to 4.

`HierarchyDefs.lean` surfaced 24 `linter.flexible` warnings and 2 `style.longLine` warnings on
removal (zero `emptyLine`/`setOption`). 6 sites (the `isUFree`/`isSFree` Bool-and pattern in
`u_free_has_single_U_type`/`s_free_has_single_U_type`) were mechanically narrowed to
`simp only [isUFree/isSFree, Bool.and_eq_true] at h` (imp/snce cases) or
`simp only [isUFree/isSFree] at h` (box case, no `Bool.and` needed); the remaining 18 flexible
sites (spanning 9 theorems: `has_single_U_type_neg/and/or`, `abstract_subst_roundtrip`,
`abstract_untl_correct`, `abstract_untl_count_le`, `abstract_snce_subst_roundtrip`,
`abstract_untl_preserves_separated`, `replace_untl_args_preserves_separated`) were narrowed to
declaration-scoped `set_option linter.flexible false in` rather than manually reconstructed,
since their goal/hypothesis-modifying `simp` calls interact with multi-step reductions
(`and_true` cleanup, nested case splits) where a literal `simp only` reconstruction risked
silently dropping a def-unfold lemma per the cycle-18 finding — this repo-wide pattern (94
pre-existing uses of `set_option linter.flexible false in` before this cycle) is an established,
sanctioned narrowing, not a new suppression. 2 longLine wraps, no semantic change.

`FrameClassVariants.lean` surfaced 61 warnings across 4 categories on removal — the largest
single-file warning count processed in this phase so far — and all 61 were resolved
*mechanically*, needing zero declaration-scoped `set_option` fallback: **38 `unusedSimpArgs`**
(dropped `Formula.and`/`Formula.or`/`Formula.neg` from `simp only [...]` calls per the linter's
own reported unused-argument list; several lines had 2-3 flagged args from the same call, so
each site's final argument set was cross-checked against *every* "unused" report for that line
before editing, not just the first); **4 `style.show`** (`show X; exact h` sites that changed
the goal via defeq unfold of `Order.succ/pred^[n+1]`, converted to `change` per the linter's own
message); **4 `linter.flexible`** (`simp; exact h(n|j)` sites proving
`Order.succ/pred^[k+1] t = s` identities, narrowed to
`simp only [Function.iterate_succ, Function.comp_apply]` — matched the exact "Try this" pattern
the build log itself suggested for a structurally identical site elsewhere in the same file;
verified by rebuild per the cycle-18 caution, not pasted blind); **15 `style.longLine`**,
mechanical wraps.

Suppression-audit progress: 246 → 256 sites audited cumulative (50 files fully processed
cumulative). Ratchet: 165 → 155 blanket suppressions repo-wide. Live per-file split: 155 total =
**141 local-only in-scope (71 files) + 14 upstream-carved-out (unchanged)**.
...
0, same 5 baseline sorry warnings plus the same one pre-existing unrelated
`backward.privateInPublic` warning in `CslibTests/FreeMonad.lean`). True sorry census
(comment-stripped, `warn.sorry`-excluded, re-derived live per "Measurement notes"): 28,
unchanged. Vacuous-def grep unchanged at the single pre-existing false positive
(`Computability/URM/Basic.lean:92`); axiom count unchanged at 26. `git show` on both of this
cycle's commits individually confirmed zero `sorry` lines touched.

**Resume point (cycle 19 close)**: the 2 files above plus all files from cycles 1, 5, 6, 7, 8, 9,
10, 11, 12, 14, 15, 16, 17, 18 are done — do not revisit (50 files total). Re-verify the
worst-offender list before starting the next cycle with the same `fs > 0`-filtered command
above. As of cycle 19's end (155 total remaining blanket suppression lines, **141 local-only
in-scope across 71 files**), **the count-6 tier is down to 4 files**:
`Temporal/Metalogic/Chronicle/CounterexampleElimination/{RecursiveWalks (1125 lines),
MainElimination (1685 lines)}.lean`, `Bimodal/Metalogic/BXCanonical/Chronicle/
CounterexampleElimination/Interface.lean` (3048 lines — do not pick this one first),
`Bimodal/Metalogic/BXCanonical/Chronicle/ChronicleToCountermodelBasic.lean` (1208 lines). Pick
`RecursiveWalks.lean` (1125 lines) next per smaller-files-first-within-tier (re-verify its line
count and suppression count with `wc -l`/`grep` before starting, since counts only ever go
down). The count-4 tier is now fully closed — do not look for count-4 files. Below the count-6
tier, a count-3/count-2/count-1 tail of roughly 66 more files remains (spot-checked in earlier
cycles, not yet individually processed).

**Done (cycle 20)**: processed 7 more files, all committed individually after a clean scoped
rebuild plus downstream-importer rebuilds — `Temporal/Metalogic/Chronicle/
CounterexampleElimination/RecursiveWalks.lean` (6→0, 1125 lines, commit `cd356658`),
`Temporal/Metalogic/Chronicle/CounterexampleElimination/MainElimination.lean` (6→0, 1685 lines,
commit `ff3d0526`), `Bimodal/Metalogic/BXCanonical/Chronicle/ChronicleToCountermodelBasic.lean`
(6→1, 1208 lines, commit `05badd91` — the 1 remaining is the correct end state: a
declaration-scoped `set_option linter.style.openClassical false in` immediately before the
file's single persistent, non-`in` `open Classical`, per the established cycle-17 finding that
this bottoms out at 1, not 0), `Bimodal/Metalogic/Separation/DualEliminations.lean` (3→0, 116
lines, commit `8107075a`), `Bimodal/Metalogic/Algebraic/ParametricHistory.lean` (3→0, 117 lines,
commit `13efd879`), `Bimodal/Metalogic/Core/MaximalConsistent.lean` (3→0, 226 lines, commit
`39720903`), `Foundations/Logic/Metalogic/Chronicle/CounterexampleElimination/Structures.lean`
(3→0, 250 lines, commit `96d4121e`). This clears the count-6 tier down to 1 file
(`Interface.lean`) and clears 4 files entirely out of the count-3 tier.

Category breakdown across the 3 large count-6-tier files (`RecursiveWalks.lean`,
`MainElimination.lean`, `ChronicleToCountermodelBasic.lean`): `style.emptyLine`/`style.setOption`
were vestigial in all three (zero warnings surfaced on removal, consistent with every prior
cycle's finding for these two categories). `style.show` surfaced in all three (23 + 51 + 11 = 85
sites total), all mechanically converted `show X` → `change X` (pure tactic-position goal
assertions, none of the term-mode `show X from e` variant, which was left untouched throughout).
`style.longLine` surfaced heavily (34 + 54 + 43 = 131 sites total across the three files),
mechanically rewrapped — mostly multi-argument `obtain ⟨...⟩ :=`/`exact ⟨...⟩` anonymous
constructors split at a comma boundary, `have h : long-type := by` splits after the type
annotation, and long declaration signatures split after a parameter group. `unusedSimpArgs`
surfaced only in `RecursiveWalks.lean` (6 sites, no line had more than one flagged argument this
time, unlike cycle 19). `linter.flexible` was vestigial in `RecursiveWalks.lean` and
`MainElimination.lean` (zero warnings on removal) but genuinely load-bearing in
`ChronicleToCountermodelBasic.lean` (7 sites, all `simp [OrderIso.symm_apply_apply]`/`simp
[OrderIso.apply_symm_apply]` calls using the *full* default simp set plus one extra lemma —
narrowed to declaration-scoped `set_option linter.flexible false in` per the established
rule-of-thumb that a bare `simp [X]` (not `simp only [X]`) is too risky to manually reconstruct
since the full default set's exact closure is unknown without interactive `simp?` output).
`style.openClassical` surfaced once in `ChronicleToCountermodelBasic.lean` (the file's single
persistent `open Classical`), restored as declaration-scoped `set_option ... in` immediately
before it, exactly matching the precedent already in `ChronicleToCountermodel.lean`.

The 4 count-3-tier files (`DualEliminations.lean`, `ParametricHistory.lean`,
`MaximalConsistent.lean`, `Structures.lean`) were each fast, single-pass fixes: `DualEliminations.lean`'s
`unusedSectionVars`/`emptyLine` were vestigial, its `unusedDecidableInType` (8 sites, one per
`_dual` theorem, all identical `[DecidableEq Atom]`-unused pattern) needed declaration-scoped
`set_option linter.unusedDecidableInType false in` per the established cycle-5 rule (never
`omit`, since these theorems' *types* don't need the instance but `omit` still risks silently
changing the elaborated signature). `ParametricHistory.lean`'s `style.show`/`style.emptyLine`
were mixed (1 genuine `show`→`change` site, `emptyLine` vestigial), `style.longLine` (3 sites)
mechanically wrapped. `MaximalConsistent.lean`'s `emptyLine`/`setOption`/`flexible` were all
three fully vestigial (zero warnings on removal — the rare case where an entire file's blanket
suppression block was dead weight). `Structures.lean`'s `setOption`/`emptyLine` were vestigial;
its 1 `linter.flexible` site (`simp at hφ` narrowing a list-membership hypothesis) was
mechanically reconstructed as `simp only [List.mem_singleton] at hφ` (simple, well-understood,
single-lemma pattern — verified by rebuild, not pasted blind).

**New parse-hazard corollary found this cycle**: when mechanically inserting
`set_option linter.X false in` immediately above a `theorem`/`def` line via a scripted
find-and-replace (as opposed to manual `Edit` calls), it is easy to insert the `set_option` line
*between* the doc comment and the declaration rather than *before* the doc comment — the same
failure mode documented at cycle 6, but now confirmed to bite scripted multi-site insertions too
(caught by rebuild before commit in `DualEliminations.lean`; corrected by a second pass moving
each `set_option` line above its preceding `/-- ... -/` block instead of between it and the
`theorem` line). Future cycles scripting multi-site `set_option ... in` insertion should insert
before the *doc comment*, matching each site's preceding `/-- -/` block, not immediately before
the declaration keyword.

Suppression-audit progress: 256 → 263 sites audited cumulative (57 files fully processed
cumulative). Ratchet (repo-wide, includes 12 upstream-carved-out): 155 → 125. Live re-derived
local-only in-scope count: **111 blanket suppressions across 64 files** (down from 141/71 at
cycle 19 close — a reduction of 30 sites / 7 files this cycle).

Full CSLib CI pipeline run once at cycle end: `lake build --wfail --iofail` (exactly 5 baseline
sorry warnings — `FrameSoundness.lean:1252`, `Intuitionistic/Scheme.lean:570,2583`,
`Intuitionistic/Completeness.lean:124`, `Minimal/Completeness.lean:118` — zero new anywhere),
`lake exe checkInitImports` (clean, no output), `lake exe lint-style` (clean, no output), `lake
lint` (checked the 7 prevention categories specifically against all 7 files touched this cycle —
zero matches; also confirmed zero `lake lint` output of any category against any of the 7 files),
`lake shake --add-public --keep-implied --keep-prefix` (exactly the 12 upstream-shared files
flagged, zero local-only findings), `lake exe mk_all --module` ("No update necessary"), `lake
test` (exit 0, same 5 baseline sorry warnings plus the same one pre-existing unrelated
`backward.privateInPublic` warning in `CslibTests/FreeMonad.lean`). Vacuous-def grep unchanged at
the single pre-existing false positive (`Computability/URM/Basic.lean:92`); axiom count unchanged
at 26. `git diff`/`git show` on all 7 of this cycle's commits individually confirmed zero `sorry`
lines touched.

**Resume point (cycle 20 close)**: the 7 files above plus all files from cycles 1, 5, 6, 7, 8, 9,
10, 11, 12, 14, 15, 16, 17, 18, 19 are done — do not revisit (57 files total). Re-verify the
worst-offender list before starting the next cycle with the same `fs > 0`-filtered command
above, gated through the upstream-exposure check. As of cycle 20's end (111 local-only in-scope
blanket suppressions across 64 files), **the count-6 tier is down to exactly 1 file**:
`Bimodal/Metalogic/BXCanonical/Chronicle/CounterexampleElimination/Interface.lean` (3048 lines —
by far the largest file remaining in this phase; still worth deferring in favor of the count-3
tier unless a cycle has a large context budget to dedicate to it specifically). The **count-3
tier now has 8 files**: `Temporal/Metalogic/MCS.lean` (538 lines),
`Temporal/Metalogic/Chronicle/ChronicleTypes.lean` (522 lines),
`Bimodal/Metalogic/Soundness/Soundness.lean` (845 lines),
`Bimodal/Metalogic/Separation/Duality.lean` (420 lines),
`Bimodal/Metalogic/Decidability/Saturation.lean` (708 lines),
`Bimodal/Metalogic/Core/RestrictedMCS.lean` (437 lines),
`Bimodal/Metalogic/Completeness.lean` (483 lines),
`Bimodal/Metalogic/BXCanonical/Chronicle/ChronicleTypes.lean` (578 lines),
`Foundations/Logic/Metalogic/Chronicle/SinceSeedConsistency.lean` (1072 lines). Pick
`Duality.lean` (420 lines) next per smaller-files-first (re-verify line count and suppression
count live before starting). Below the count-3 tier, a count-2/count-1 tail remains (not yet
individually surveyed this cycle; re-derive with the same command before assuming its size).

Apply the same method: remove all of a file's suppressions, rebuild, categorize what surfaces
into vestigial / mechanically-fixable / needs-real-proof-work, fix what's mechanical, narrow the
rest to declaration-scope, re-baseline the ratchet in the same commit — carrying forward every
safety lesson from cycles 6-20 (see the cycle-11/12 addenda above for the full list through cycle
12, the cycle-14 declaration-mapping-verification lesson, the cycle-15 Edit-tool
false-duplicate-match lesson, the cycle-16 mirrored-proof-block `replace_all` lesson, the
cycle-17 `openClassical`/`unusedArguments` findings, the cycle-18 "Try this" trace-verification
finding, the cycle-19 finding (collect every `unusedSimpArgs` warning for a line before editing
it), and this cycle's two findings: (1) a bare `simp [X]` (not `simp only [X]`) pulling in the
full default simp set is a strong signal to prefer declaration-scoped
`set_option linter.flexible false in` over manual reconstruction, since the full set's closure
isn't visible without interactive `simp?` output; (2) when scripting multi-site
`set_option ... in` insertion, insert above the preceding doc comment, not between it and the
declaration — the parse hazard from cycle 6 recurs with scripted insertion. Also confirmed as
safe and repo-precedented: declaration-scoped `set_option linter.flexible false in` and
`set_option linter.unusedDecidableInType false in`, and file-level-but-declaration-scoped
`set_option linter.style.openClassical false in` immediately before a persistent `open Classical`
when replacing every use site individually is out of this phase's scope (statement-preservation
mandate). Only one `unusedSimpArgs` warning per line surfaced this cycle, but the cycle-19
"collect every warning for that line first" rule remains the correct general procedure — do not
assume single-argument warnings going forward without checking. Also confirmed as safe and
repo-precedented (94+ pre-existing uses before this cycle): declaration-scoped
`set_option linter.flexible false in`, placed immediately before the declaration's doc comment,
is an acceptable narrowing for `linter.flexible` sites whose goal/hypothesis-modifying `simp`
interacts with multi-step reductions (or pulls in the full default simp set via a bare
`simp [X]`) where a literal `simp only` reconstruction risks dropping a def-unfold lemma —
prefer manual `simp only` reconstruction only when the pattern is simple and well-understood
(e.g. a single Bool-and unfold, a single `List.mem_singleton`-style membership unfold, or a
`Function.iterate_succ`/`Function.comp_apply` identity matching an existing "Try this" hint
elsewhere in the same file), but do not force it when the risk of a silent semantic gap is high.

**Effort estimate for future cycles**: this cycle cleared 10 sites (2 files) in one dispatch —
below the ~20-30 sites/cycle blended historical average, because `FrameClassVariants.lean`'s 61
raw warnings (across 4 categories, the largest single-file count processed in this phase) took
disproportionate effort per suppression-count-unit relative to its own `fs=6` starting count.
Files with many mechanically-fixable `unusedSimpArgs`/`longLine` warnings can still be
slow in wall-clock terms even though every fix is safe and low-risk. At a blended rate accounting
for this variance, clearing the remaining **141 local-only sites** is roughly **6-9 more
cycles** — the count-6 tier's remaining 4 files (all 900+ lines, `Interface.lean` at 3048 lines)
should be treated as an upper-bound-effort tier; the count-3/2/1 tail below it has historically
gone faster per file.

**Effort estimate for future cycles (cycle 20 update)**: this cycle cleared 30 sites (7 files) in
one dispatch — well above the blended historical average, because the two largest count-6-tier
files (`RecursiveWalks.lean`, `MainElimination.lean`) were both mostly mechanical
(`show`→`change` plus `longLine` rewraps) with the correctness-adjacent categories
(`unusedSimpArgs`, `flexible`) either absent or vestigial, and the 4 count-3-tier files processed
were each fast single-pass fixes (small, well-scoped files). At this rate, clearing the remaining
**111 local-only sites across 64 files** is roughly **4-6 more cycles**, but this estimate should
be treated cautiously — cycle 19 showed the same variance the other direction
(`FrameClassVariants.lean`'s 61-warning single file). `Interface.lean` (3048 lines, the sole
remaining count-6-tier file) is the single largest remaining unknown and should be budgeted as
possibly consuming most or all of a dedicated cycle by itself once the count-3/2/1 tail is
cleared, or tackled early by a cycle with a large context budget specifically reserved for it.

**Done (cycle 21)**: processed 5 files, all in the count-3 tier, all 3→0 clean:
`Separation/Duality.lean` (420 lines), `Core/RestrictedMCS.lean` (437 lines),
`Completeness.lean` (483 lines), `Temporal/Metalogic/Chronicle/ChronicleTypes.lean` (522 lines),
`Temporal/Metalogic/MCS.lean` (538 lines). See the RESUME HERE section's cycle-21 findings for
the per-file detail; not repeated here.

**Done (cycle 22, this dispatch — final cycle of this `/orchestrate` invocation)**: processed 10
files, clearing the count-3 tier entirely and opportunistically starting the count-2 tier once
the count-3 worklist ran dry. All 10 committed individually after a clean scoped rebuild plus
downstream-importer rebuilds: `Bimodal/Metalogic/Soundness/Soundness.lean` (845→869 lines, 3→0,
commit `37ac7f38` — 25 `unusedSimpArgs` sites reduced to `simp only [truthAt]`/`[Formula.neg]`,
26 `longLine` wraps), `Bimodal/Metalogic/Decidability/Saturation.lean` (708 lines, 3→0, commit
`efb08c0e` — 15 `longLine` wraps + 1 bare-`simp` `flexible` site narrowed to declaration scope),
`Bimodal/Metalogic/BXCanonical/Chronicle/ChronicleTypes.lean` (578 lines, 3→0, commit `74fec4d4`
— the Bimodal counterpart of the Temporal file closed cycle 21; 14 `dupNamespace` sites narrowed
on the `Chronicle` structure/c0-c5' conditions + ~39 `longLine` wraps), `Foundations/Logic/
Metalogic/Chronicle/SinceSeedConsistency.lean` (1072 lines, 3→0, commit `1a14739f` — 7
`flexible` sites across 5 declarations narrowed; `setOption`/`unusedSimpArgs` both proved
unnecessary), `Bimodal/Metalogic/Bundle/FMCSDef.lean` (43 lines, 2→0, commit `13515a71`),
`Bimodal/Metalogic/BXCanonical/CanonicalChain.lean` (92 lines, 2→0, commit `3d50f988`),
`Bimodal/FrameConditions/Compatibility.lean` (105 lines, 2→0, commit `a2c7f2f1`),
`Bimodal/Metalogic/Bundle/Construction.lean` (123 lines, 2→0, commit `16d312b4`),
`Bimodal/Metalogic/Bundle/UntilSinceCoherence.lean` (127 lines, 2→0, commit `c0a2c843` — the
file's 2 pre-existing `warn.sorry`-guarded sorries, blocked on upstream continuous-frame
completeness, were left completely untouched), `Bimodal/Metalogic/Bundle/BFMCS.lean` (130 lines,
2→0, commit `c8fa93ef`). Also re-baselined the `check-lint-suppressions.sh` ratchet
(`851278c7`), which had drifted stale across several prior cycles — now at 86 blanket
suppressions across 63 files repo-wide (includes the 12 upstream-carved-out; the local-only
in-scope figure is 72 across 49 files). See the RESUME HERE section's cycle-22 entry for the
full per-file finding detail, the live re-derived count-2/count-1 tier survey, and the effort
estimate for future cycles — not repeated here.

**Done (cycle 23, resumed via `/implement` from the paused `/orchestrate` invocation)**:
processed 17 files, clearing 9 of the count-2 tier's smallest files plus 8 count-1-tier files
(2 more count-1-tier files inspected — `Conversions.lean`, `DefectChain.lean` — turned out to be
pre-existing, documented permanent exceptions and were left untouched). All 17 committed
individually after a clean scoped rebuild plus downstream-importer rebuilds (two heavily-imported
core files verified via full `lake build` instead): `Temporal/Metalogic/Chronicle/
ChronicleToCountermodel.lean` (137 lines, 2→0, commit `36173000`), `Bimodal/Metalogic/Algebraic/
ParametricCompleteness.lean` (143 lines, 2→0, commit `6d7d0899`), `Bimodal/Metalogic/
Decidability/Correctness.lean` (147 lines, 2→0, commit `27eb4e34` — `omit [DecidableEq Atom]
[Hashable Atom] in` on `decide_sound`), `Bimodal/Metalogic/BXCanonical/
OrderedSeedConsistency.lean` (149 lines, 2→0, commit `451bdc0d`), `Bimodal/Metalogic/Algebraic/
ParametricCanonical.lean` (156 lines, 2→0, commit `765da621`), `Bimodal/Metalogic/Algebraic/
InteriorOperators.lean` (176 lines, 2→0, commit `192c115f` — 5 `show`→`change` sites, a new
category), `Bimodal/Metalogic/Bundle/ModalSaturation.lean` (207 lines, 2→0, commit `9b88abae` —
18 `longLine` wraps), `Bimodal/Metalogic/Bundle/CanonicalFrame.lean` (267 lines, 2→0, commit
`b4f639b1` — 12 `longLine` wraps), `Bimodal/Metalogic/Bundle/TemporalContent.lean` (173 lines,
2→0, commit `9373a025` — not on cycle 22's tracked worklist, surfaced live), `Temporal/Metalogic/
Chronicle/CanonicalChain.lean` (76 lines, 1→0, commit `467b2cd3`), `Temporal/ProofSystem/
Derivation.lean` (98 lines, 1→0, commit `4af7dc81`), `Bimodal/Metalogic/BXCanonical/Quasimodel/
SubformulaClosure.lean` (99 lines, 1→0, commit `f9914e41`), `Bimodal/FrameConditions/
Soundness.lean` (117 lines, 1→0, commit `992088df`), `Bimodal/FrameConditions/Validity.lean` (118
lines, 1→0, commit `527ce26d`), `Temporal/Metalogic/Chronicle/OrderedSeedConsistency.lean` (135
lines, 1→0, commit `ca6b79cc`), `Bimodal/Metalogic/Separation/IntHelpers.lean` (159 lines, 1→0,
commit `fb8b4976`), `Bimodal/ProofSystem/Derivation.lean` (168 lines, 1→0, commit `0d6a9f19` —
verified via full `lake build`, 3309/3309, not an enumerated importer list, since it is the
shared `DerivationTree` base type for most of the Bimodal tree). Also re-baselined the
`check-lint-suppressions.sh` ratchet (`274e07ce`) — now 60 blanket suppressions across 46 files
repo-wide (includes the 14 upstream-carved-out; the local-only in-scope figure is 46 across 32
files). See the RESUME HERE section's cycle-23 entry for the full per-file finding detail, the
live re-derived count-2/count-1 tier survey (including the two newly-identified permanent
exceptions), and the effort estimate for future cycles — not repeated here.
...
- ~~Suppression audit outcome recorded per site (repo-wide, ~570).~~ **Restated (upstream-exposure
  ...
  than edited here. **[PARTIAL]** — 304 sites done (85 files fully processed cumulative); **46
  blanket suppressions across 32 local-only files remain in scope** (60 repo-wide, so 14 in
