...
/-! # Modal KB5 as a Conservative Extension of Classical Propositional Logic

This module proves that Modal KB5 is a conservative extension of Classical Propositional Logic
(CPL): any propositional formula `φ` that is derivable in KB5 (as `φ.toModal`) is already
derivable in CPL.

The proof uses the universal model `(Unit, fun _ _ => True, fun _ => v)`, which satisfies
all KB5 frame conditions (symmetry: `fun _ _ _ => trivial` and Euclideanness:
...
- `kb5_conservative_extension`: If `φ.toModal` is KB5-derivable then `φ` is CPL-derivable.
...
/-- **Modal KB5 Conservative Extension over CPL**:

If the modal translation `φ.toModal` is derivable in KB5, then `φ` is derivable
in Classical Propositional Logic.

Proof: construct the universal model `(Unit, fun _ _ => True, fun _ => v)`.
Symmetry and Euclideanness hold vacuously. KB5 soundness gives satisfaction
...
theorem kb5_conservative_extension {Atom : Type*} {φ : PL.Proposition Atom}
  ...
  exact (modal_satisfies_toModal_iff_evaluate m () φ).mp
    (kb5_soundness d m (fun _ _ _ => trivial) (fun _ _ _ _ _ => trivial)
      () (fun _ h => nomatch h))
