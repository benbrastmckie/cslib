...
| C4 | `LTree`,`star`,`fullSubtree`,`prune` + the **unfolding identity** (§2.3); DELETE `pathTo`/`pathToList`/`Star_append` | `#eval`/`decide` reproduces Simpson's worked example verbatim | MED — **[COMPLETED]** this dispatch, `probes/lemma612-scaffold.lean` (extended in place, per the resume-pointer's deferred choice). **`star` FIXED**: was a double-`bigAnd` (`(bigAnd labels).and(bigAnd children)`), now a single `bigAnd` over the CONCATENATED list (labels ++ children-diamonds) — the old version produced a spurious `⊤∧(◇(⊤∧⊤))` at a label-less single-childed node instead of the source's plain `◇⊤`, which would have FAILED the worked-example check. `prune`/`fullSubtree` added (children split as `pre ++ [c]`, continuation child last, matching `addChild`'s existing append convention — forward-compatible with C5). **Unfolding identity proved as an IK-derivable two-way implication (`star_unfold_imp1`/`star_unfold_imp2`), NOT raw `Eq`**: `bigAnd`'s right-fold makes literal term equality fail once the pruned node has 2+ "ordinary" children (associativity mismatch, `a.and(X.and y) ≠ (a.and X).and y` as TERMS despite being logically equivalent) — confirmed by hand-checking the general case before writing any Lean. Built via a new reusable list-level lemma `bigAnd_append_singleton_imp1`/`imp2` (induction on `xs`, using new `andI_deriv`/`andE1_deriv`/`andE2_deriv`/`top_deriv`/`bigAnd_cons_of_ne_nil` combinators). **Success criterion met via a real theorem, not literal `#eval`/`decide`**: `Label` has no computable `DecidableEq` in this file (only the classically-opaque `Classical.propDecidable` local instance), so `star_Star_worked_example` proves the verbatim match (`((◇A⊃□□B)∧◇A) ⊃ □(◇⊤⊃◇B)`, tree `x→y,x→z→w`, target `z`) via `simp`-driven filter unfolding rather than `rfl`/`decide`. `pathTo`/`pathToList`/`Star_append` all DELETED (confirmed `Star_append` was solving the pruning problem with the wrong primitive, per report 02 §2.3); `Star`/`Star_imp1`/`Star_imp2`/`box_mono1`/`box_mono2`/`IK.impIntro`/`NIKAx`/`TClosure.hilbertTransport` all UNCHANGED and reverified compiling (they only ever reference `star Γ t` opaquely, never unfold its equation, so were not at regression risk). Sorry-free, axiom footprint `[propext, Classical.choice, Quot.sound]` (verified via `#print axioms` on `star_unfold_imp1`/`star_unfold_imp2`/`star_Star_worked_example`/`bigAnd_append_singleton_imp1`/`imp2` — no new axioms). C1/C2/C3's separate file (`probes/track-c-c1-tele-conj.lean`) untouched and reverified compiling. |
