...
instance :
    HasAxiomAndI Modal.HilbertT
      (F := Modal.Proposition Atom) where
  andI := ⟨Modal.DerivationTree.ax [] _ (Modal.TAxiom.andI _ _)⟩

instance :
    HasAxiomAndE1 Modal.HilbertT
      (F := Modal.Proposition Atom) where
  andE1 := ⟨Modal.DerivationTree.ax [] _ (Modal.TAxiom.andE1 _ _)⟩

instance :
    HasAxiomAndE2 Modal.HilbertT
      (F := Modal.Proposition Atom) where
  andE2 := ⟨Modal.DerivationTree.ax [] _ (Modal.TAxiom.andE2 _ _)⟩

instance :
    HasAxiomOrI1 Modal.HilbertT
      (F := Modal.Proposition Atom) where
  orI1 := ⟨Modal.DerivationTree.ax [] _ (Modal.TAxiom.orI1 _ _)⟩

instance :
    HasAxiomOrI2 Modal.HilbertT
      (F := Modal.Proposition Atom) where
  orI2 := ⟨Modal.DerivationTree.ax [] _ (Modal.TAxiom.orI2 _ _)⟩

instance :
    HasAxiomOrE Modal.HilbertT
      (F := Modal.Proposition Atom) where
  orE := ⟨Modal.DerivationTree.ax [] _ (Modal.TAxiom.orE _ _ _)⟩
instance :
