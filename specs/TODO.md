...
557 [NOT STARTED] — Refactor and restructure the modal Tableau subsystem to library-p
...
**Description**: Refactor and restructure the modal Tableau subsystem to library-publication quality, identifying the correct abstractions and module divisions, archiving unnecessary code to a new Boneyard/ quarantine, and systematically discharging the documentation debt. MOTIVATION: four successive soundness routes for the S4 keyed loop-check guard have failed, each at the same obligation but by a different mechanism (Route P: redirect-inertness lemmas machine-checked FALSE at a reachable state; origin-edge invariant revision: abandoned; ancestor-only blocking: the branchSatisfiableIn witness model is existentially arbitrary, forcing an uncontrollable transitive closure; subtractive blocking with a completeness-only redirect channel: the free transfer yields only an UNWRAPPED branch fact at the redirect target and unwrapped facts have NO PERSISTENCE MECHANISM in this tableau's Hintikka apparatus). The last of these is a statement about the design, not the route. This task treats the factoring as a first-class defect rather than continuing to route around it. It is NOT a soundness-proof task and must not attempt the soundness obligation. EVIDENCE OF MIS-FACTORING, all measured: (1) AMPLIFICATION -- the entire semantic dependence on the redirect edge is 17 lines in 4 clauses (LoopChecking.lean around 6557-6562, 8756-8759, 8763-8766), yet changing it requires 4 declarations / 1,036 lines of new proof, with 43 declarations / 1,983 lines reachable from modalTableauS4Keyed_complete. A well-factored design localises a 17-line semantic change to roughly that neighbourhood; a 60x amplification means the semantics is smeared across the proof structure. (2) BRIDGE ACCUMULATION -- ten hintikkaS4_* bridge lemmas cluster at one interface; six had their hypotheses weakened and two more were added during the subtractive attempt because conjuncts required a wrapped antecedent where the existing bridges produce the unwrapped form. Adapters multiplying at one boundary is the signature of a wrong abstraction: each consumer needs a slightly different shim because the underlying predicate is not factored around what consumers need. (3) FILE SIZE -- LoopChecking.lean is 10,674 lines / 150 declarations, FrameSoundness.lean 5,317, FrameCompleteness.lean 4,532; 20,523 lines in three files, in a project that ships ORGANISATION.md. (4) DEAD WEIGHT RETAINED BY DESIGN -- the S4LoopInv field outDegEq is provided at two sites and consumed nowhere in the S4 line, and its 188-line preservation lemma was deliberately left in place because removing it carried regression risk. (5) DUPLICATION WITHOUT A RETIREMENT PATH -- the abandoned plan v4 states twice that no landed driver is retired, so its parallel-Sub-family strategy had no terminal consolidation phase; parallel families are correct for landing a proof and wrong as an end state. (6) DOCUMENTATION DRIFT -- an asserted axiom count of 26 did not reproduce (47 raw matches measured), so the files' own documentation has diverged from the code. SCOPE. A. ABSTRACTIONS AND UNIFICATION: identify the correct abstraction boundary at the Hintikka/saturation interface and collapse the ten-bridge adapter set behind it. The known unification target is the S5 apply-parametric ladder modalTableauGen / modalExpandBranchesGen, which currently threads neither keys nor any redirect record -- extending that generic mechanism to thread per-driver state is the principled alternative to forking a parallel family per driver. Evaluate whether the wrapped-versus-unwrapped formula distinction deserves to be an explicit abstraction (a persistence-carrying predicate) rather than an implicit property that four routes discovered the hard way; the absence of a persistence mechanism for unwrapped facts is the recurring obstruction and may be the single most valuable thing to name. DO NOT INVENT THIS ABSTRACTION FROM SCRATCH -- it already has a standard name and definition in the literature, and the corpus holds the source. Chagrov and Zakharyaschev, Modal Logic (Oxford Logic Guides 35, 1997), now ingested and fidelity-verified at ratio 1.0209: the interval theorem (chunk_0246.md lines 43-65) states explicitly that a relation S between S and S-bar may be NONTRANSITIVE even when the original R is transitive, which is precisely the failure mode a subtractive or redirect-channel design runs into; and the standard remedy is the Lemmon filtration box-plus operator, box-plus(phi) = phi AND box(phi) (chunk_0248.md lines 24-31), which IS the wrapped-plus-unwrapped pairing. Theorem 5.51 (chunk_0267.md) builds a model whose relation is the reflexive-transitive closure of the generated edges and discharges box-propagation through a containment of the generated relation in the ambient one. Read p02_kripke-semantics.md for the chapter-level treatment of canonical models, filtration and selective filtration. Cross-check against Blackburn, de Rijke and Venema chapters 2 and 6 for a second independent presentation. The task-facing question is therefore not "what should this abstraction be" but "does the Lemmon box-plus pairing, or a Lean formulation of it, factor this subsystem correctly, and what does adopting it cost the landed proofs". B. MODULE DIVISION: split the three oversized files along the boundaries the abstraction analysis identifies, conforming to ORGANISATION.md and NOTATION.md, preserving import acyclicity, and keeping lake exe checkInitImports green. Do not split mechanically by line count; splits must follow real seams. C. BONEYARD: create Boneyard/ at the repository root -- it does NOT currently exist here; it is a convention borrowed from the source repository, where it held roughly 27k lines and 29 sorries and was excluded from porting, from sorry censuses, and from the build. Establish and document that convention explicitly in Boneyard/README.md: quarantined, never imported by Cslib/, excluded from lake build, mk_all, lint-style, shake, and all sorry/axiom censuses, retained for provenance rather than use. Move there, rather than deleting: machinery orphaned by the four dead routes, the dead outDegEq field and its 188-line preservation proof once its zero-consumer status is re-verified, and any surviving Sub-family remnants that no theorem consumes. Nothing whose deletion cannot be justified by a re-verified zero-consumer check may be moved, and nothing proven and consumed may be moved at all. D. DOCUMENTATION: systematically discharge the documentation debt. Repository-wide there are 11 TODO: and 8 NOTE: tags in Cslib/, and NOTABLY ZERO FIX:/TODO:/NOTE:/QUESTION: tags in the three modal Tableau files -- so the debt there is not tag-shaped but prose-shaped: module comments and docstrings that state intent, known limitations, or obstructions and have since drifted from the code or describe work never done. Concrete instances: the branchPropAdequateIn module comment documenting the Route P obstruction; LoopChecking.lean around 2022 documenting lemmas that are FALSE as stated rather than merely unproven; LoopChecking.lean around 7513 asserting all ten invariant fields are closed with zero sorry; FrameCompleteness.lean 4174-4177 stating the S4 decidability instance remains out of scope until both a genuine soundness theorem and the completeness theorem exist for the same driver; FrameSoundness.lean 1193 and 1215 documenting a retained sorry. Each such statement must be verified against the code and then either implemented, corrected, or restated as an accurate limitation with its evidence. The axiom-count drift must be fixed by capturing a measured baseline rather than asserting a number. CONSTRAINTS. Never edit Rules.lean, Saturation.lean (ModalTableauResult is consumed by 8 files and carries only (b, acc)), or Branch.lean without an explicit consumer audit first. Preserve every proven, consumed result: this is a restructuring task and must be behaviour-preserving on all landed theorems, with modalTableauS4Keyed_complete and the six landed Decidable instances (K/T/B/S5/Five/KB5) green at every commit. The sorry at FrameSoundness.lean:1244 is retained by explicit user decision and its disposition is a separate decision, not this task's to make. Route-independent assets deliberately kept from the subtractive attempt -- modalS4Saturated, the six strictly-weakened hintikkaS4 bridges, hasEdge_accWithReds_iff, reflTransGen_accWithReds_first_red, and the two sorry-free blockedRedirect_unwrapped_{boxPos,diaNeg}_mem transfers -- are inputs to be placed correctly by the abstraction analysis, not candidates for the Boneyard. Run the CSLib vetting pipeline against CONTRIBUTING.md, NOTATION.md, ORGANISATION.md, and CODE_OF_CONDUCT.md as an acceptance gate; it has never been run on this subsystem. VERIFICATION: behaviour preservation demonstrated by the landed capstones and Decidable instances remaining green, the sorry census in Cslib/Logics/Modal/Tableau/ not increasing, no new axioms, checkInitImports and lint-style clean, and the existing executable regression corpora (CslibTests/S4LoopGuardRegression.lean and the probe harnesses under the S4 loop-guard task's artifacts directory) reproducing their recorded verdicts exactly. Expect this task to need expansion into several tasks; the abstraction analysis should be completed and reviewed before any file is moved or split.
