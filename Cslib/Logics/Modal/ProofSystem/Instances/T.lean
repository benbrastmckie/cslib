...
/-- Local forward-direction discharge from `SchemaUnion` to `TAxiom`, used by
the instance registrations below. Mirrors the `.mp` half of
`Cslib.Logics.Modal.ProofSystem.SchemaBridges.schemaUnion_tTags_iff_TAxiom`;
...
`inductive TAxiom` it targets, once the redefinition makes the conversion a
defeq no-op. -/
private theorem tTags_of_schemaUnion {χ : Proposition Atom}
    (h : SchemaUnion (insert .implyK <| insert .implyS <| insert .efq <| insert .peirce <|
      ...
      <| insert .diaDualityBack <| insert .modalT ∅ :
      Finset ModalSchemaTag) χ) : TAxiom χ := by
