...
def liveReport (rs : List (P × Option Bool × Option Bool)) : IO Unit := do
  ...
  IO.println s!"-- LIVE driver, 2 atoms, size<=5, fuel 60 (total={rs.length})"
...
#eval liveReport (liveRows 2 5 60)

/-! ### Transition matrix, including the fuel-exhausted (`none`) cells.

The `openToClosed`/`closedToOpen` counters above deliberately ignore `none`. Box-plus decides
strictly more formulas within the same fuel cap, so `none -> some _` transitions exist; each one
must be checked against a HIGH-fuel reference verdict, or an unsound closure could hide behind a
reference fuel exhaustion. -/

def noneToClosed (rs : List (P × Option Bool × Option Bool)) : List P :=
  (rs.filter (fun r => r.2.1 == none && r.2.2 == some true)).map Prod.fst

def noneToOpen (rs : List (P × Option Bool × Option Bool)) : List P :=
  (rs.filter (fun r => r.2.1 == none && r.2.2 == some false)).map Prod.fst

def someToNone (rs : List (P × Option Bool × Option Bool)) : List P :=
  (rs.filter (fun r => r.2.1 != none && r.2.2 == none)).map Prod.fst

def auditReport (rs : List (P × Option Bool × Option Bool)) : IO Unit := do
  let n2c := noneToClosed rs
  let n2o := noneToOpen rs
  let s2n := someToNone rs
  IO.println s!"none -> CLOSED : {n2c.length}   (each re-checked at reference fuel 600 below)"
  IO.println s!"none -> OPEN   : {n2o.length}"
  IO.println s!"some -> none   : {s2n.length}   (regression if >0)"
  let mut bad := 0
  for f in n2c do
    match closesLiveRef f 600 with
    | some false => bad := bad + 1; IO.println s!"  UNSOUND CANDIDATE (ref OPEN at fuel 600, plus CLOSED): {ppP f}"
    | some true  => pure ()
    | none       => IO.println s!"  ref still fuel-exhausted at 600: {ppP f}"
  IO.println s!"unsound candidates = {bad}"

#eval auditReport (liveRows 2 5 60)
