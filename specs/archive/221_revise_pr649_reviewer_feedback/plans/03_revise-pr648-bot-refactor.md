...
- **Artifacts**: plans/03_revise-pr648-bot-refactor.md (this file)
- **Standards**: plan-format.md, status-markers.md, artifact-management.md, tasks.md
- **Type**: pr

## Overview

PR #648 (feat/propositional-v2) adds bot as a primitive constructor to the propositional formula type, eliminating `[Bot Atom]` constraints. Since PR #536 merged (refactoring IsIntuitionistic/IsClassical to inference-system-based), the branch has 3 merge conflicts and requires reconciliation of the Theory section. Additionally, reviewers requested removing Semantics files to a follow-up PR, replacing German references with English alternatives, and keeping upstream's `impl` constructor naming. Definition of done: branch rebased on upstream/main, all conflicts resolved, Semantics files removed, German references replaced, `lake build` passes, PR description revised addressing ctchou and thomaskwaring feedback.
...
- **reports/02_pr648-branch-analysis.md** (v2): Detailed branch diff analysis of feat/propositional-v2 vs upstream/main post-#536. Identified 3 merge conflicts (Cslib.lean imports, Defs.lean imports, Defs.lean Theory section), documented the bot refactor specifics, and produced a step-by-step reconciliation plan. Key finding: the constructor naming decision should favor upstream's `impl` (not `imp`) to avoid contention.
...
- Adopt upstream's `impl`/`implI`/`implE` constructor naming (revert rename from `imp`)
...
- Addressing Modal formula `impl` naming (separate concern)
...
| Constructor rename reversion misses references in NaturalDeduction/Basic.lean | M | L | grep for `impI`/`impE`/`andE1`/`andE2`/`orI1`/`orI2` after revert to catch stragglers |
| Reviewer still objects to primitive bot after balanced rationale | M | M | ctchou supports the change; focus PR description on constraint-elimination benefits |
...
- [ ] Verify auto-merged `NaturalDeduction/Basic.lean` accepted branch's constructor names (to be reverted in Phase 2)
...
### Phase 2: Remove Semantics Files and Reconcile Constructor Naming [NOT STARTED]

**Goal**: Remove Semantics files from PR scope per reviewer request, and revert the `imp`->`impl` constructor rename to match upstream convention.
...
- [ ] In `Defs.lean`: revert constructor from `imp` to `impl` in the `Proposition` inductive type
- [ ] In `Defs.lean`: update all references to `.imp` -> `.impl` (derived connectives `neg`, `top`, `iff`, `subst`)
- [ ] In `NaturalDeduction/Basic.lean`: revert `impI`->`implI`, `impE`->`implE`, `andE1`->`andE₁`, `andE2`->`andE₂`, `orI1`->`orI₁`, `orI2`->`orI₂`
- [ ] Verify `Theory.lean` (from #536) now uses consistent constructor names with Basic.lean
- [ ] In `Connectives.lean`: rename `HasImp` to match convention if needed (check if upstream #607 uses `HasImpl`; if so align, otherwise keep `HasImp` for now)
...
- `Cslib/Logics/Propositional/Defs.lean` - revert `imp` to `impl`
- `Cslib/Logics/Propositional/NaturalDeduction/Basic.lean` - revert constructor names

**Verification**:
- Semantics files no longer exist on branch
- `grep -r "Semantics.Basic\|Semantics.Bool" Cslib/` returns no matches
- `grep -r "\.imp\b" Cslib/Logics/Propositional/` returns no matches (all converted to `.impl`)
- `lake build` succeeds
...
- [ ] In `NaturalDeduction/Basic.lean`: verify `derivationTop`, `derivableIn_top`, `derivable_iff_equiv_top` have no `[Inhabited Atom]` constraint (since top = impl bot bot, no constraint needed)
...
- [ ] Explain `impl` constructor naming: kept upstream convention (`impl`), consistent with Modal formula type and merged #536
...
- Constructor naming justified by upstream convention
...
- [ ] Constructor naming consistent: `impl`/`implI`/`implE` throughout
- [ ] PR description covers all reviewer feedback categories
- [ ] Avigad2022 entry present in references.bib with correct metadata

## Artifacts & Outputs

- `specs/221_revise_pr649_reviewer_feedback/plans/03_revise-pr648-bot-refactor.md` (this plan)
