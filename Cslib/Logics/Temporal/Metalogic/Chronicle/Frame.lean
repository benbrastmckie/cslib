...
structure TPoint (Atom : Type*) where
  /-- The set of formulas constituting this canonical point. -/
...
theorem g_content_set_consistent {Omega : Set (Formula Atom)}
  ...
  -- h_G_bot : G(⊥) ∈ Omega is not defeq to ¬F(⊤) ∈ Omega since G is primitive; convert via
...
theorem h_content_set_consistent {Omega : Set (Formula Atom)}
  ...
  -- h_H_bot : H(⊥) ∈ Omega is not defeq to ¬P(⊤) ∈ Omega since H is primitive; convert via
  -- mcs_allPast_iff.
...
lemma tForwardWitness (w : TPoint Atom) (ψ : Formula Atom)
    (h_F : (𝐅ψ) ∈ w.formulas) :
...
lemma tBackwardWitness (w : TPoint Atom) (ψ : Formula Atom)
    (h_P : (𝐏ψ) ∈ w.formulas) :
