...
- **Modal** (4): `modalK` (K distribution), `modalD` (seriality), `modalFour` (transitivity),
  `modalFive` (Euclideanness)
- plus the and/or/diamond-duality characterization tags, all part of `kCore`. -/
abbrev D45Axiom : Proposition Atom → Prop := SchemaUnion d45Tags
...
instance :
    HasAxiomDiamondDualityFwd Modal.HilbertD45
      (F := Modal.Proposition Atom) where
  diamondDualityFwd := ⟨Modal.DerivationTree.ax [] _
    (⟨.diamondDualityFwd, by decide, _, rfl⟩)⟩

instance :
    HasAxiomDiamondDualityBack Modal.HilbertD45
      (F := Modal.Proposition Atom) where
