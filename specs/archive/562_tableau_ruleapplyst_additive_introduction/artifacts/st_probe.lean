import Cslib.Logics.Modal.Tableau.Saturation
...
theorem modalExpandBranchesGen_eq_St (apply : RuleApply Atom)
  ...
  | zero =>
    ...
    cases hf : (branches.zip accs).findSome?
        (fun x => if isModalClosed x.fst = true then none else some (x.fst, x.snd)) with
    | none => rfl
    | some p => obtain ⟨b, a⟩ := p; rfl
  | succ fuel' ih =>
...
#print axioms Cslib.Logic.Modal.Tableau.modalStepBranchGen_eq_St
#print axioms Cslib.Logic.Modal.Tableau.modalExpandBranchesGen_eq_St
#print axioms Cslib.Logic.Modal.Tableau.modalTableauGen_eq_St
#print axioms Cslib.Logic.Modal.Tableau.findSome?_map_comm
end Probe
