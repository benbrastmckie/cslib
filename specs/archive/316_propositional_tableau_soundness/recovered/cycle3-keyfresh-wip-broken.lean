  ...
  | succ k ih =>
    ...
    · simp only [hcond, ite_true, Option.some.injEq] at hfilt
      ...
      · -- New edge: (ch, source) = (nw, parentLabel), so ch = nw, source = parentLabel
        -- Compute helper facts before destructuring (so we can reference nw and parentLabel by name)
  ...
  · simp only [heq12, Bool.false_eq_true, ite_false] at hacc
    ...
    · -- w2 ≠ nw
      ...
      · -- w1 = nw: nw has no children in old edges, so DFS from nw can't reach w2 ≠ nw
        ...
        rw [isAccessible_go_nw_no_children edges w1 w2 hnw_not_parent (edges.length + 1)] at hacc_old
              ...
              | some step =>
                ...
                | linearResult newForms nw' newEdge =>
                  ...
                  · -- linearResult bp∈bt case: bp is in the tail bt with edges edgesT;
                    ...
                    have hfreshCombLin : ∀ b e nw,
                      ...
                      rw [show done ++ [Branch.extendMany bPers newForms] ++ bt =
                              (done ++ [Branch.extendMany bPers newForms]) ++ bt from
                            List.append_assoc _ _ _,
                          show doneEdges ++ [edges'] ++ edgesT =
                              (doneEdges ++ [edges']) ++ edgesT from List.append_assoc _ _ _,
                          show doneNW ++ [nw'] ++ nwT =
                              (doneNW ++ [nw']) ++ nwT from List.append_assoc _ _ _] at hmem
                      rw [List.zip_append hlen1, List.zip_append hlen2,
                          List.mem_append] at hmem
                ...
                | branchingResult branches' nw' =>
                  ...
                  have hfreshCombBr : ∀ b e nw,
                    ...
                    intro b e nw hmem
                    have hlenBr : branches'.length = branches'.length := le_refl _
                    ...
                    have hlen2 : (done ++ branches'.map (Branch.extendMany bPers ·)).length =
                      ...
                      simp; omega
                    rw [show done ++ branches'.map (Branch.extendMany bPers ·) ++ bt =
                            (done ++ branches'.map (Branch.extendMany bPers ·)) ++ bt from
                          List.append_assoc _ _ _,
                        show doneEdges ++ branches'.map (fun _ => edgesH) ++ edgesT =
                            (doneEdges ++ branches'.map (fun _ => edgesH)) ++ edgesT from
                          List.append_assoc _ _ _,
                        show doneNW ++ branches'.map (fun _ => nwH) ++ nwT =
                            (doneNW ++ branches'.map (fun _ => nwH)) ++ nwT from
                          List.append_assoc _ _ _] at hmem
                    rw [List.zip_append hlen1, List.zip_append hlen2,
                        List.mem_append] at hmem
                    rcases hmem with h_front | h_back
                    · have hlen_de : done.length = doneEdges.length := by omega
