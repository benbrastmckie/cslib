...
/-! ## S4 Subtractive Route (3), Phase 2 (Decision Gate A): the `red`-Threaded Truth Lemma

`specs/553_s4_loop_guard_soundness_reachability_restriction/plans/
04_subtractive-blocking-red-channel.md`, Phase 2 tasks (c)-(d). Consumes Phase 1's bifurcated
`modalHintikkaSetS4Sub` (`LoopChecking.lean`) and the path-decomposition lemma
`reflTransGen_accWithReds_first_red` (`LoopChecking.lean`), reusing `extractModelS4` (this file,
`:143-189`) verbatim at `accWithReds acc red` -- no `extractModelS4Sub` is needed, since
`Accessibility` is a bare edge list (`Branch.lean:55-57`). -/

/-- Modal Truth Lemma for the S4 subtractive route: membership in a `modalHintikkaSetS4Sub`
branch tracks satisfaction in the extracted reflexive-transitive Kripke model
`extractModelS4 b (accWithReds acc red)`.

Transcribes `modalTruthLemmaS4`'s complexity induction verbatim for every case except
box-positive and diamond-negative: the propositional cases (`atom`/`bot`/`imp`/`and`/`or`) and
the box-negative/diamond-positive witness cases depend only on conjunct 2 (`modalS4Saturated`,
evaluated over `acc` alone -- never `accWithReds acc red`, per the Phase 1 module doc) and
conjuncts 3/4 (the witnesses, already stated over `accWithReds acc red` in
`modalHintikkaSetS4Sub`), so they carry over unchanged with the model's accessibility argument
switched to `accWithReds acc red`.

The box-positive and diamond-negative cases decompose the model's `ReflTransGen (accWithReds
acc red)` path via `reflTransGen_accWithReds_first_red`: an `acc`-only path uses the weakened
bridge `hintikkaS4_box_pos_reflTransGen`/`hintikkaS4_dia_neg_reflTransGen` (now over
`modalS4Saturated`) directly, exactly as in `modalTruthLemmaS4`; a path with a first `red`-hop
`(x, wB, s, φ) ∈ red` uses the same weakened bridge to carry the *boxed*/*diamond-negated*
formula only as far as `x` (the `red`-hop's source), then discharges the residual cone `wB ⤳
w'` via `modalHintikkaSetS4Sub`'s forward-cone conjunct 5 (`G*`)/conjunct 6 (`F*`) -- **never**
the wrapped-at-target form, which has 40 measured counterexamples (see the Phase 1 module doc
in `LoopChecking.lean`). -/
lemma modalTruthLemmaS4Sub
    (φ₀ : Proposition Atom) (b : List (SignedFormula (Proposition Atom) WorldIndex))
    (acc : Accessibility) (red : Reds Atom) (hH : modalHintikkaSetS4Sub φ₀ b acc red) :
    ∀ (φ : Proposition Atom) (w : WorldIndex),
      (⟨.pos, φ, w⟩ ∈ b → Satisfies (extractModelS4 b (accWithReds acc red)) w φ) ∧
      (⟨.neg, φ, w⟩ ∈ b → ¬ Satisfies (extractModelS4 b (accWithReds acc red)) w φ) := by
  suffices H : ∀ (n : Nat) (φ : Proposition Atom), modalComplexity φ = n → ∀ w,
      (⟨.pos, φ, w⟩ ∈ b → Satisfies (extractModelS4 b (accWithReds acc red)) w φ) ∧
      (⟨.neg, φ, w⟩ ∈ b → ¬ Satisfies (extractModelS4 b (accWithReds acc red)) w φ) by
    intro φ w; exact H (modalComplexity φ) φ rfl w
  ...
  | ind n IHn =>
    intro φ hφ w
    have IH : ∀ (ψ : Proposition Atom), modalComplexity ψ < n → ∀ w',
        (⟨.pos, ψ, w'⟩ ∈ b → Satisfies (extractModelS4 b (accWithReds acc red)) w' ψ) ∧
        (⟨.neg, ψ, w'⟩ ∈ b → ¬ Satisfies (extractModelS4 b (accWithReds acc red)) w' ψ) :=
      fun ψ hlt w' => IHn (modalComplexity ψ) hlt ψ rfl w'
    have hHopen : isModalClosed b = false := hH.1
    have hHrule := hH.2.1
    cases φ with
    ...
    | box ψ =>
      ...
      · intro hmem w' hr
        have hpath :
            Relation.ReflTransGen (fun a c => (accWithReds acc red).hasEdge a c = true) w w' :=
          extractModelS4_r b (accWithReds acc red) ▸ hr
        rcases reflTransGen_accWithReds_first_red acc red w w' hpath with
          hleft | ⟨x, wB, s, φR, hpre, hmemred, hsuf⟩
        · exact (IH ψ (by rw [← hφ]; simp only [modalComplexity_box]; omega) w').1
            (hintikkaS4_box_pos_reflTransGen φ₀ b acc hHrule ψ w w' hmem hleft)
        · have hTboxAtX : (⟨.pos, .box ψ, x⟩ : SignedFormula (Proposition Atom) WorldIndex) ∈ b :=
            hintikkaS4_box_pos_reflTransGen_boxed φ₀ b acc hHrule ψ w x hmem hpre
          have hTψAtW' : (⟨.pos, ψ, w'⟩ : SignedFormula (Proposition Atom) WorldIndex) ∈ b :=
            hH.2.2.2.2.1 ψ x wB s φR hmemred hTboxAtX w' hsuf
          exact (IH ψ (by rw [← hφ]; simp only [modalComplexity_box]; omega) w').1 hTψAtW'
      · intro hmem hall
        obtain ⟨w', hw', hF⟩ := hH.2.2.1 ψ w hmem
        exact (IH ψ (by rw [← hφ]; simp only [modalComplexity_box]; omega) w').2 hF
          (hall w' (extractModelS4_hasEdge_imp_r b (accWithReds acc red) hw'))
    | diamond ψ =>
      constructor
      · intro hmem
        obtain ⟨w', hw', hT⟩ := hH.2.2.2.1 ψ w hmem
        exact ⟨w', extractModelS4_hasEdge_imp_r b (accWithReds acc red) hw',
          (IH ψ (by rw [← hφ]; simp only [modalComplexity_diamond]; omega) w').1 hT⟩
      · intro hmem
        rintro ⟨w', hw', hsψ⟩
        have hpath :
            Relation.ReflTransGen (fun a c => (accWithReds acc red).hasEdge a c = true) w w' :=
          extractModelS4_r b (accWithReds acc red) ▸ hw'
        rcases reflTransGen_accWithReds_first_red acc red w w' hpath with
          hleft | ⟨x, wB, s, φR, hpre, hmemred, hsuf⟩
        · exact (IH ψ (by rw [← hφ]; simp only [modalComplexity_diamond]; omega) w').2
            (hintikkaS4_dia_neg_reflTransGen φ₀ b acc hHrule ψ w w' hmem hleft) hsψ
        · have hFdiaAtX :
              (⟨.neg, .diamond ψ, x⟩ : SignedFormula (Proposition Atom) WorldIndex) ∈ b :=
            hintikkaS4_dia_neg_reflTransGen_boxed φ₀ b acc hHrule ψ w x hmem hpre
          have hFψAtW' : (⟨.neg, ψ, w'⟩ : SignedFormula (Proposition Atom) WorldIndex) ∈ b :=
            hH.2.2.2.2.2 ψ x wB s φR hmemred hFdiaAtX w' hsuf
          exact (IH ψ (by rw [← hφ]; simp only [modalComplexity_diamond]; omega) w').2 hFψAtW' hsψ

/-- An open S4 subtractive Hintikka branch with `F(φ₀)@0 ∈ b` yields a reflexive-transitive
Kripke countermodel to `φ₀` over `accWithReds acc red`. Transcription of
`modalOpenBranchS4_countermodel` at `accWithReds acc red`, reusing `extractModelS4_refl`/
`extractModelS4_trans` verbatim (both are generic in the accessibility argument). -/
theorem modalOpenBranchS4Sub_countermodel
    (φ₀ : Proposition Atom) (b : List (SignedFormula (Proposition Atom) WorldIndex))
    (acc : Accessibility) (red : Reds Atom)
    (hH : modalHintikkaSetS4Sub φ₀ b acc red)
    (hF : (⟨.neg, φ₀, 0⟩ : SignedFormula (Proposition Atom) WorldIndex) ∈ b) :
    (¬ Satisfies (extractModelS4 b (accWithReds acc red)) 0 φ₀) ∧
      s4FC (extractModelS4 b (accWithReds acc red)).r :=
  ⟨(modalTruthLemmaS4Sub φ₀ b acc red hH φ₀ 0).2 hF,
    ⟨extractModelS4_refl b (accWithReds acc red), extractModelS4_trans b (accWithReds acc red)⟩⟩
