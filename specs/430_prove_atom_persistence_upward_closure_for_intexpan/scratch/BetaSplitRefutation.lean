...
(already positive at world `1`), giving a second route to the same containment. -/
def phiRef2 : Proposition Nat :=
  ((pr ∨ ps) ∧ (((pr ∨ ps) → ((pr ∨ ps) → pr)) → pb)) → pr

/-- Variant: one more nesting level between the reuse target and the reused-into world. -/
...
more to match on (tests robustness of the construction rather than its core). -/
def phiRef4 : Proposition Nat :=
  ((pr ∨ ps) ∧ (pc ∧ ((ps → (ps → pr)) → pb))) → pr

#eval report phiRef1 40
#eval atomTable phiRef1 40
#eval report phiRef2 40
#eval report phiRef3 40
...
/-- Per-world forced atoms at the real fuel. -/
def tableAtRealFuel := atomTable phiRef1 realFuel
...
/-- Readable dump of the returned branch: `(sign-is-positive, formula, label)`. -/
...
#eval minAtomTable
#eval reportMin phiRef3 realFuel
#eval reportMin phiRef4 realFuel
