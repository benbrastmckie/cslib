...
/-- Local forward-direction discharge from `SchemaUnion` to `D45Axiom`, used by
the instance registrations below. Mirrors the `.mp` half of
`Cslib.Logics.Modal.ProofSystem.SchemaBridges.schemaUnion_d45Tags_iff_D45Axiom`;
...
`inductive D45Axiom` it targets, once the redefinition makes the conversion a
defeq no-op. -/
private theorem d45Tags_of_schemaUnion {χ : Proposition Atom}
    (h : SchemaUnion (insert .implyK <| insert .implyS <| insert .efq <| insert .peirce <|
      ...
      insert .modalFive ∅ :
      Finset ModalSchemaTag) χ) : D45Axiom χ := by
