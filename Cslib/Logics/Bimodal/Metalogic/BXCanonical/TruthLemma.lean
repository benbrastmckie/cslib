...
- **bot**: Trivial (bot not in any MCS, and truthAt gives False)
- **imp**: MCS implication property iff material conditional
- **box**: Modal witness construction (bxModalWitness)
- **allFuture (G)**: bx_G_forward + bxGBackward
- **allPast (H)**: bx_H_forward + bxHBackward
...
`bxUntilEventualityResolution` / `bxSinceEventualityResolution` in Frame.lean.
...
theorem G_iff_mcs (w : BXPoint Atom) (φ : Formula Atom) :
    Formula.allFuture φ ∈ w.formulas ↔ ∀ v : BXPoint Atom, bxLe w v → φ ∈ v.formulas := by
  constructor
  · intro h_G v h_le
    exact bx_G_forward h_le h_G
  · intro h_all
    by_contra h_not_G
    obtain ⟨v, h_le, h_not_φ⟩ := bxGBackward w φ h_not_G
...
theorem H_iff_mcs (w : BXPoint Atom) (φ : Formula Atom) :
    Formula.allPast φ ∈ w.formulas ↔ ∀ v : BXPoint Atom, bxLe v w → φ ∈ v.formulas := by
  constructor
  · intro h_H v h_le
    exact bx_H_forward h_le h_H
  · intro h_all
    by_contra h_not_H
    obtain ⟨v, h_le, h_not_φ⟩ := bxHBackward w φ h_not_H
    exact h_not_φ (h_all v h_le)
...
theorem box_iff_mcs (w : BXPoint Atom) (φ : Formula Atom) :
    Formula.box φ ∈ w.formulas ↔
      ∀ v : BXPoint Atom, bxModalEquiv w v → φ ∈ v.formulas := by
  constructor
  · intro h_box v h_equiv
    ...
    exact SetMaximalConsistent.implication_property v.is_mcs
      (theoremInMcsFc v.is_mcs h_ax) h_box_v
  · intro h_all
    by_contra h_not_box
    ...
    have h_dia_neg : Formula.diamond φ.neg ∈ w.formulas :=
      SetMaximalConsistent.implication_property w.is_mcs
        (theoremInMcsFc w.is_mcs h_neg_box_to_dia) h_neg_box
    obtain ⟨v, h_equiv, h_neg_in⟩ := bxModalWitness w φ.neg h_dia_neg
...
def bxLt (w v : BXPoint Atom) : Prop :=
  bxLe w v ∧ ¬bxLe v w

/-! ### Helper: F(psi) from witness existence -/

/-- If bxLe w v, psi in v, then F(psi) in w. -/
...
/-- If bxLe v w, psi in v, then P(psi) in w. Mirror of F_from_witness. -/
...
theorem until_forward_mcs (w : BXPoint Atom) (φ ψ : Formula Atom)
  ...
  · exact Or.inl h_φ
  · exact Or.inr (bxUntilEventualityResolution w ψ φ h_until h_φ)
...
theorem since_forward_mcs (w : BXPoint Atom) (φ ψ : Formula Atom)
  ...
  by_cases h_φ : φ ∈ w.formulas
  · exact Or.inl h_φ
  · exact Or.inr (bxSinceEventualityResolution w ψ φ h_since h_φ)
