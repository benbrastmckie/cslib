...
/-! # Strong Completeness for Modal Logic TB

This module proves strong soundness and strong completeness for modal logic TB:
semantic entailment from a set of premises `Gamma` (over all reflexive, symmetric frames)
is equivalent to set-derivability using `TBAxiom`.

## Main Results

- `tb_strong_soundness`: Set-derivability from `Gamma` implies semantic entailment
  over all reflexive, symmetric frames.
...
* Cslib/Logics/Modal/Metalogic/Systems/TB/Completeness.lean -- TB completeness (weak)
-/
