...
instance :
    HasAxiomAndI Modal.HilbertD
      (F := Modal.Proposition Atom) where
  andI := ⟨Modal.DerivationTree.ax [] _ (Modal.DAxiom.andI _ _)⟩

instance :
    HasAxiomAndE1 Modal.HilbertD
      (F := Modal.Proposition Atom) where
  andE1 := ⟨Modal.DerivationTree.ax [] _ (Modal.DAxiom.andE1 _ _)⟩

instance :
    HasAxiomAndE2 Modal.HilbertD
      (F := Modal.Proposition Atom) where
  andE2 := ⟨Modal.DerivationTree.ax [] _ (Modal.DAxiom.andE2 _ _)⟩

instance :
    HasAxiomOrI1 Modal.HilbertD
      (F := Modal.Proposition Atom) where
  orI1 := ⟨Modal.DerivationTree.ax [] _ (Modal.DAxiom.orI1 _ _)⟩

instance :
    HasAxiomOrI2 Modal.HilbertD
      (F := Modal.Proposition Atom) where
  orI2 := ⟨Modal.DerivationTree.ax [] _ (Modal.DAxiom.orI2 _ _)⟩

instance :
    HasAxiomOrE Modal.HilbertD
      (F := Modal.Proposition Atom) where
  orE := ⟨Modal.DerivationTree.ax [] _ (Modal.DAxiom.orE _ _ _)⟩
