...
-/
def isValid (D : Type) [AddCommGroup D] [LinearOrder D] [IsOrderedAddMonoid D]
    (φ : Formula Atom) : Prop :=
...
theorem valid_at_triple {φ : Formula Atom} (ℱ : TaskFrame D) (M : TaskModel Atom ℱ)
    (Omega : Set (WorldHistory ℱ)) (_h_sc : ShiftClosed Omega)
    (τ : WorldHistory ℱ) (_h_mem : τ ∈ Omega) (t : D) (h_valid : isValid D φ) :
    truthAt M Omega τ t φ := h_valid ℱ M Omega _h_sc τ _h_mem t
