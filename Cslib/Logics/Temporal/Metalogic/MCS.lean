...
/-- Or resolution: if `(φ ∨ ψ) ∈ Ω` and `(¬φ) ∈ Ω` then `ψ ∈ Ω`.

Proof sketch:
1. Derive `⊢ ¬φ → (φ → ψ)`: from context `[¬φ, φ]` get `⊥` (modus ponens), then `ψ` (efq),
   then apply deduction theorem twice.
2. Apply `temporal_implication_property` with `h_neg` to get `(φ → ψ) ∈ Ω`.
3. Derive `⊢ (φ → ψ) → ((ψ → ψ) → ((φ ∨ ψ) → ψ))` from `or_elim` axiom.
4. Derive `⊢ ψ → ψ` (identity axiom chain) to get `(ψ → ψ) ∈ Ω`.
5. Chain implication properties twice to get `((φ ∨ ψ) → ψ) ∈ Ω`.
6. Apply `temporal_implication_property` with `h_or` to get `ψ ∈ Ω`. -/
theorem temporal_or_resolve_left
    {Ω : Set (Formula Atom)} (h_mcs : Temporal.SetMaximalConsistent Ω)
    {φ ψ : Formula Atom} (h_or : (Formula.or φ ψ) ∈ Ω) (h_neg : (Formula.neg φ) ∈ Ω) :
    ψ ∈ Ω := by
  -- Build ⊢ ¬φ → (φ → ψ) by DT twice from context [¬φ, φ] ⊢ ψ
  have d_neg_to_imp : DerivationTree FrameClass.Base [] (φ.neg.imp (φ.imp ψ)) := by
    let ctx := [φ, φ.neg]
    have d_bot : DerivationTree FrameClass.Base ctx Formula.bot :=
    ...
    have d_psi : DerivationTree FrameClass.Base ctx ψ :=
      .modus_ponens ctx Formula.bot ψ
        ...
        d_bot
    exact deductionTheorem [] φ.neg (φ.imp ψ) (deductionTheorem [φ.neg] φ ψ d_psi)
  -- Get (φ → ψ) ∈ Ω from ¬φ ∈ Ω
  have h_phi_to_psi : (φ.imp ψ) ∈ Ω :=
    temporal_implication_property h_mcs (theoremInMcs h_mcs d_neg_to_imp) h_neg
  -- Build ⊢ ψ → ψ (identity via K and S axioms: MP (MP K[ψ,ψ→ψ,ψ] S[ψ,ψ→ψ]) S[ψ,ψ])
  -- K[ψ,ψ→ψ,ψ] : (ψ→(ψ→ψ)→ψ) → ((ψ→ψ→ψ) → (ψ→ψ))
  -- S[ψ,ψ→ψ]   : ψ → (ψ→ψ) → ψ
  -- S[ψ,ψ]     : ψ → ψ → ψ
  have d_id : DerivationTree FrameClass.Base [] (ψ.imp ψ) :=
    .modus_ponens [] _ _
      (.modus_ponens [] _ _
        (.axiom [] _ (.imp_k ψ (ψ.imp ψ) ψ) trivial)
        (.axiom [] _ (.imp_s ψ (ψ.imp ψ)) trivial))
      (.axiom [] _ (.imp_s ψ ψ) trivial)
  -- Get ((ψ → ψ) → ((φ ∨ ψ) → ψ)) ∈ Ω via or_elim axiom and h_phi_to_psi
  have h_id_to_or :
      ((ψ.imp ψ).imp ((Formula.or φ ψ).imp ψ)) ∈ Ω :=
    temporal_implication_property h_mcs
      (theoremInMcs h_mcs (.axiom [] _ (.or_elim φ ψ ψ) trivial))
      h_phi_to_psi
  -- Get ((φ ∨ ψ) → ψ) ∈ Ω via (ψ → ψ) ∈ Ω
  have h_or_to_psi : ((Formula.or φ ψ).imp ψ) ∈ Ω :=
    temporal_implication_property h_mcs h_id_to_or (theoremInMcs h_mcs d_id)
  -- Conclude ψ ∈ Ω
  exact temporal_implication_property h_mcs h_or_to_psi h_or
