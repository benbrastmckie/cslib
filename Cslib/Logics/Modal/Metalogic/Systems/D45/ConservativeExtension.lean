...
/-! # Modal D45 as a Conservative Extension of CPL -/
...
/-- Modal D45 is a conservative extension of CPL: if `φ.toModal` is D45-derivable then `φ` is
...
theorem d45_conservative_over_cpl {Atom : Type*} {φ : PL.Proposition Atom}
  ...
  modal_conservative_extension_param h fun _ => by
    obtain ⟨d⟩ := h
    exact d45_soundness d _ ⟨fun w => ⟨w, trivial⟩⟩ (fun _ _ _ _ _ => trivial)
