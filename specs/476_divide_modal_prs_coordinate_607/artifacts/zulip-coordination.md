...
**Discovered issue in #607 itself** (task 477, follow-up task 485): while reworking #662 onto a both-primitive basis stacked on #607, the whole-library build was found to fail on #607's own tip — `Cslib/Logics/HML/LogicalEquivalence.lean:105` still instantiates the **old 3-arg** `LogicalEquivalence` class, but #607's commit `059a85c1` ("parametrised logical equivalence") upgraded that class to a **4-arg** signature (adds the inference-system param `S`) without updating this HML instantiation site (its last real change was a mathlib bump). Confirmed pre-existing and independent of #662 via `git stash` isolation on a pristine `pr607` base. This blocks `checkInitImports`/`shake`/`test` for the entire library (all pull in the `Cslib.lean` aggregator). Verify this is still present before mentioning it (fmontesi may fix it upstream first). Task 485 tracks a narrow instance migration if a local fix is preferred.
...
One small heads-up from doing that refactor: `Cslib/Logics/HML/LogicalEquivalence.lean:105` still uses the old 3-arg `LogicalEquivalence`, which no longer typechecks against the parametrised (4-arg) class #607 introduces — that instantiation site looks like it was missed when the class was parametrised, and it currently breaks the whole-library build on #607's tip (independent of my modal work). A small instance migration should sort it; happy to push that as a tiny PR against #607 if that's easiest.
