...
/-- A multiset `Y` is Dershowitz–Manna strictly smaller than `X + {a}` when every element of
`Y` is strictly less than `a` (regardless of the common prefix `X`). -/
private theorem Multiset.isDershowitzMannaLT_all_lt_add {a : ℕ} {X Y : Multiset ℕ}
    (hY : ∀ y ∈ Y, y < a) :
    Multiset.IsDershowitzMannaLT Y (X + {a}) :=
  ⟨∅, Y, X + {a}, by simp, by simp, by simp, fun y hy => ⟨a, by simp, hY y hy⟩⟩
...
private theorem Theory.Derivation.reduceRoot_decreases_normMeasure
  ...
  · -- h_1: impE (impI _ D) E  →  D.subsOne E   (substitution β)
    ...
    · simp [hE_mf] at h
    · rw [h]; simp only [Proposition.complexity]; omega
  ...
  · -- h_4: orE _ (orI1 _ D) DA _  →  DA.subsOne D   (disjunction β)
    rename_i _ _ A1 B1 Dinj DAb DBc
    rw [Option.some.injEq] at hd'; subst hd'
    ...
    simp only [Multiset.add_zero, Multiset.zero_add, Multiset.empty_eq_zero]
    rw [Multiset.add_comm]
    apply Multiset.isDershowitzMannaLT_all_lt_add
    intro y hy
  ...
  · -- h_5: orE _ (orI2 _ D) _ DB  →  DB.subsOne D
    rename_i _ _ A1 B1 Dinj DAc DBb
    rw [Option.some.injEq] at hd'; subst hd'
    rcases h_subsSN with ⟨hD_sn, hDB_sn⟩
    have hD_mf := maximalFormulas_sn_eq_zero _ hD_sn
    have hDB_mf := maximalFormulas_sn_eq_zero _ hDB_sn
    refine Prod.Lex.left _ _ ?_
    simp only [normMeasure, maximalFormulas, conclusionComplexity, hD_mf, hDB_mf]
    simp only [Multiset.add_zero, Multiset.zero_add, Multiset.empty_eq_zero]
    rw [Multiset.add_comm]
    apply Multiset.isDershowitzMannaLT_all_lt_add
    intro y hy
    have hynew : y ∉ DBb.maximalFormulas := by simp [hDB_mf]
    rcases subsOne_new_redex_complexity_lt DBb Dinj hy hynew with h | h
    · simp [hD_mf] at h
    · rw [h]; simp only [Proposition.complexity]; omega
  · -- h_6: andE1 G (orE _ D DA DB)  →  orE G D (andE1 DA) (andE1 DB)   (commuting)
    rename_i _ _ D' DA' DB'
    rw [Option.some.injEq] at hd'; subst hd'
    rcases h_subsSN with ⟨hA, hB⟩
    -- After substitution, show normMeasure as pairs and apply Prod.Lex.right.
    -- maximalFormulas equality: DA' and DB' are not andI (excluded by the SN side condition).
    show Prod.Lex Multiset.IsDershowitzMannaLT (· < ·)
      ((orE _ D' (andE1 _ DA') (andE1 _ DB')).maximalFormulas,
       (orE _ D' (andE1 _ DA') (andE1 _ DB')).commutingSum)
      ((andE1 _ (orE _ D' DA' DB')).maximalFormulas,
       (andE1 _ (orE _ D' DA' DB')).commutingSum)
    -- DA' ≠ andI and DA' ≠ orE (since andE1 DA' is SN)
    have hDA'_mf : (andE1 _ DA').maximalFormulas = DA'.maximalFormulas := by
      simp only [isStronglyNormal] at hA
      cases DA' <;> simp_all [maximalFormulas]
    have hDA'_cs : (andE1 _ DA').commutingSum = DA'.commutingSum := by
      simp only [isStronglyNormal] at hA
      cases DA' <;> simp_all [commutingSum]
    -- DB' ≠ andI and DB' ≠ orE (since andE1 DB' is SN)
    have hDB'_mf : (andE1 _ DB').maximalFormulas = DB'.maximalFormulas := by
      simp only [isStronglyNormal] at hB
      cases DB' <;> simp_all [maximalFormulas]
    have hDB'_cs : (andE1 _ DB').commutingSum = DB'.commutingSum := by
      simp only [isStronglyNormal] at hB
      cases DB' <;> simp_all [commutingSum]
    rw [show (orE _ D' (andE1 _ DA') (andE1 _ DB')).maximalFormulas =
           (andE1 _ (orE _ D' DA' DB')).maximalFormulas from by
      -- (andE1 G (orE G D' DA' DB')).maximalFormulas = (orE G D' DA' DB').maximalFormulas (orE ≠ andI)
      simp only [maximalFormulas]
      -- (andE1 DA').maximalFormulas = DA'.maximalFormulas (DA' ≠ andI from hA)
      rw [hDA'_mf, hDB'_mf]
      cases D' <;> simp [maximalFormulas]]
    refine Prod.Lex.right _ ?_
    simp only [commutingSum, hDA'_cs, hDB'_cs]
    cases D' <;> simp_all [commutingSum, nodeCount] <;> omega
  · -- h_7: andE2 G (orE _ D DA DB)  →  orE G D (andE2 DA) (andE2 DB)
    rename_i _ _ D' DA' DB'
    rw [Option.some.injEq] at hd'; subst hd'
    rcases h_subsSN with ⟨hA, hB⟩
    show Prod.Lex Multiset.IsDershowitzMannaLT (· < ·)
      ((orE _ D' (andE2 _ DA') (andE2 _ DB')).maximalFormulas,
       (orE _ D' (andE2 _ DA') (andE2 _ DB')).commutingSum)
      ((andE2 _ (orE _ D' DA' DB')).maximalFormulas,
       (andE2 _ (orE _ D' DA' DB')).commutingSum)
    -- DA' ≠ andI and DA' ≠ orE (since andE2 DA' is SN)
    have hDA'_mf : (andE2 _ DA').maximalFormulas = DA'.maximalFormulas := by
      simp only [isStronglyNormal] at hA
      cases DA' <;> simp_all [maximalFormulas]
    have hDA'_cs : (andE2 _ DA').commutingSum = DA'.commutingSum := by
      simp only [isStronglyNormal] at hA
      cases DA' <;> simp_all [commutingSum]
    -- DB' ≠ andI and DB' ≠ orE (since andE2 DB' is SN)
    have hDB'_mf : (andE2 _ DB').maximalFormulas = DB'.maximalFormulas := by
      simp only [isStronglyNormal] at hB
      cases DB' <;> simp_all [maximalFormulas]
    have hDB'_cs : (andE2 _ DB').commutingSum = DB'.commutingSum := by
      simp only [isStronglyNormal] at hB
      cases DB' <;> simp_all [commutingSum]
    rw [show (orE _ D' (andE2 _ DA') (andE2 _ DB')).maximalFormulas =
           (andE2 _ (orE _ D' DA' DB')).maximalFormulas from by
      simp only [maximalFormulas]
      rw [hDA'_mf, hDB'_mf]
      cases D' <;> simp [maximalFormulas]]
    refine Prod.Lex.right _ ?_
  ...
  · -- h_8: impE (orE _ D DA DB) E  →  orE G D (impE DA E') (impE DB E')
    rename_i _ _ D' DA' DB' E'
    rw [Option.some.injEq] at hd'; subst hd'
    rcases h_subsSN with ⟨hA, hB⟩
    -- E' is SN (from hA/hB), so E'.maximalFormulas = ∅.
    have hE'_sn : E'.isStronglyNormal = true := by
      simp only [isStronglyNormal] at hA
      split at hA <;> simp_all [isStronglyNormal, Bool.and_eq_true]
    have hE'_mf : E'.maximalFormulas = ∅ := maximalFormulas_sn_eq_zero _ hE'_sn
    show Prod.Lex Multiset.IsDershowitzMannaLT (· < ·)
      ((orE _ D' (DA'.impE (weakCtx _ E')) (DB'.impE (weakCtx _ E'))).maximalFormulas,
       (orE _ D' (DA'.impE (weakCtx _ E')) (DB'.impE (weakCtx _ E'))).commutingSum)
      (((orE _ D' DA' DB').impE E').maximalFormulas,
       ((orE _ D' DA' DB').impE E').commutingSum)
    -- DA' ≠ impI and DA' ≠ orE (since impE DA' (weakCtx E') is SN)
    have hDA'_mf : (DA'.impE (weakCtx _ E')).maximalFormulas =
        DA'.maximalFormulas + E'.maximalFormulas := by
      simp only [isStronglyNormal] at hA
      cases DA' <;> simp_all [maximalFormulas, maximalFormulas_weakCtx]
    have hDA'_cs : (DA'.impE (weakCtx _ E')).commutingSum =
        DA'.commutingSum + (weakCtx _ E').commutingSum := by
      simp only [isStronglyNormal] at hA
      cases DA' <;> simp_all [commutingSum]
    have hDB'_mf : (DB'.impE (weakCtx _ E')).maximalFormulas =
        DB'.maximalFormulas + E'.maximalFormulas := by
      simp only [isStronglyNormal] at hB
      cases DB' <;> simp_all [maximalFormulas, maximalFormulas_weakCtx]
    have hDB'_cs : (DB'.impE (weakCtx _ E')).commutingSum =
        DB'.commutingSum + (weakCtx _ E').commutingSum := by
      simp only [isStronglyNormal] at hB
      cases DB' <;> simp_all [commutingSum]
    rw [show (orE _ D' (DA'.impE (weakCtx _ E')) (DB'.impE (weakCtx _ E'))).maximalFormulas =
           ((orE _ D' DA' DB').impE E').maximalFormulas from by
      -- impE (orE ...) E' doesn't match impI pattern, so .mf = (orE ...).mf + E'.mf
      simp only [maximalFormulas]
      rw [hDA'_mf, hDB'_mf, hE'_mf, Multiset.add_zero]
      cases D' <;> simp [maximalFormulas]]
    refine Prod.Lex.right _ ?_
    simp only [commutingSum, hDA'_cs, hDB'_cs]
    cases D' <;> simp_all [commutingSum, nodeCount] <;> omega

/-! ## Well-Founded Existence of Strongly Normal Forms -/

/-- Every derivation has a strongly normal form.

Proved by well-founded induction on the lexicographic triple
`(maximalFormulas d, commutingSum d, nodeCount d)` (Dershowitz–Manna × ℕ × ℕ, all WF):
- For β-redexes the primary `maximalFormulas` component strictly decreases.
- For commuting conversions the secondary `commutingSum` component strictly decreases.
- For structural recursion into subterms the tertiary `nodeCount` strictly decreases.
([Prawitz1965], Ch. III–IV; [Troelstra-Schwichtenberg2000] §6.1.8.) -/
private theorem Theory.Derivation.exists_stronglyNormal_form
    {G : Ctx Atom} {A : Proposition Atom} (d : T.Derivation G A) :
    ∃ d' : T.Derivation G A, d'.isStronglyNormal = true := by
  -- Well-founded induction on the triple (maximalFormulas, commutingSum, nodeCount).
  -- We phrase it as: it suffices to prove the statement for all (triple, G, A, d) with
  -- (d.mf, d.cs, d.nc) = triple, by well-founded induction on triple.
  suffices ∀ (triple : Multiset ℕ × ℕ × ℕ) {G : Ctx Atom} {A : Proposition Atom}
      (d : T.Derivation G A), (d.maximalFormulas, d.commutingSum, d.nodeCount) = triple →
      ∃ d' : T.Derivation G A, d'.isStronglyNormal = true from this _ rfl
  intro triple
  apply (WellFounded.prod_lex Multiset.wellFounded_isDershowitzMannaLT
    (WellFounded.prod_lex Nat.lt_wfRel.wf Nat.lt_wfRel.wf)).induction
    (C := fun triple => ∀ {G : Ctx Atom} {A : Proposition Atom} (d : T.Derivation G A),
      (d.maximalFormulas, d.commutingSum, d.nodeCount) = triple →
      ∃ d' : T.Derivation G A, d'.isStronglyNormal = true)
  intro triple ih G A d h
  -- We need the IH in the form: for any d' with strictly smaller triple, ∃ SN form.
  have ih_d : ∀ (G' : Ctx Atom) (A' : Proposition Atom) (d' : T.Derivation G' A'),
      Prod.Lex Multiset.IsDershowitzMannaLT (Prod.Lex (· < ·) (· < ·))
        (d'.maximalFormulas, d'.commutingSum, d'.nodeCount)
        (d.maximalFormulas, d.commutingSum, d.nodeCount) →
      ∃ e : T.Derivation G' A', e.isStronglyNormal = true := by
    intro G' A' d' hlt
    rw [h] at hlt
    exact ih _ hlt rfl
  -- Short-circuit: if d is already SN, return it.
  by_cases h_sn : d.isStronglyNormal = true
  · exact ⟨d, h_sn⟩
  -- d is not SN. Case-split on the constructor.
  induction d with
  | ax hax => simp [isStronglyNormal] at h_sn
  | ass has => simp [isStronglyNormal] at h_sn
  | andI G D₁ D₂ ih₁ ih₂ =>
    simp only [isStronglyNormal, Bool.and_eq_true, Bool.not_eq_true] at h_sn
    push_neg at h_sn
    -- At least one of D₁, D₂ is not SN; normalize both recursively via ih_d.
    have ⟨D₁', hD₁'⟩ := ih_d _ _ D₁ (by
      simp only [nodeCount, maximalFormulas, commutingSum] at h ⊢
      apply Prod.Lex.right; apply Prod.Lex.right; omega)
    have ⟨D₂', hD₂'⟩ := ih_d _ _ D₂ (by
      simp only [nodeCount, maximalFormulas, commutingSum] at h ⊢
      apply Prod.Lex.right; apply Prod.Lex.right; omega)
    exact ⟨andI G D₁' D₂', by simp [isStronglyNormal, hD₁', hD₂']⟩
  | andE1 G D ihD =>
    simp only [isStronglyNormal] at h_sn
    -- Get SN form of D
    have ⟨D', hD'_sn⟩ := ih_d _ _ D (by
      simp only [nodeCount, commutingSum, maximalFormulas] at h ⊢
      rcases D with _ | _ | _ | _ | _ | _ | _ | _ | _ | _ <;>
        simp_all [maximalFormulas, commutingSum, nodeCount] <;>
        first
        | (apply Prod.Lex.left; exact Multiset.isDershowitzMannaLT_cons_add)
        | (apply Prod.Lex.right; apply Prod.Lex.right; omega)
        | (apply Prod.Lex.right; apply Prod.Lex.right; omega)
        | (apply Prod.Lex.right; apply Prod.Lex.right; omega))
    -- Case-split on D' to see if andE1 G D' is SN
    rcases D' with _ | _ | G_and D₁ D₂ | G_and D' | G_and D' | _ | _ | G_or D'' DA DB | _ | D_fun E_arg
    -- ax, ass, andE1, andE2, orI1, orI2, impI: andE1 G D' is SN since D' ≠ andI, D' ≠ orE, D'.sn
    all_goals first
    | exact ⟨andE1 G _, by simp_all [isStronglyNormal]⟩
    -- andI case: beta redex, reduce to D₁
    | (rename_i D₁ D₂
       simp only [isStronglyNormal, Bool.and_eq_true] at hD'_sn
       exact ⟨D₁, hD'_sn.1⟩)
    -- orE case: commuting conversion
    | (rename_i D'' DA DB
       -- D' = orE G_or D'' DA DB is SN: D''.sn ∧ DA.sn ∧ DB.sn ∧ D'' ≠ orI1/orI2/orE
       simp only [isStronglyNormal] at hD'_sn
       -- Get SN form of andE1 _ DA
       have ⟨e₁, he₁⟩ : ∃ e : T.Derivation _ _, e.isStronglyNormal = true := by
         rcases DA with _ | _ | G_and2 E₁ E₂ | _ | _ | _ | _ | _ | _ | _
         -- DA = andI: andE1 _ (andI G_and2 E₁ E₂) → E₁ (beta)
         all_goals first
         | (simp [isStronglyNormal] at hD'_sn ⊢; exact ⟨_, hD'_sn.1⟩)
         | exact ⟨andE1 _ _, by simp_all [isStronglyNormal]⟩
         | (apply ih_d
            simp only [nodeCount, commutingSum, maximalFormulas] at h ⊢
            split_ifs <;> simp_all [maximalFormulas, commutingSum, nodeCount, isStronglyNormal] <;>
              first
              | (apply Prod.Lex.right; apply Prod.Lex.right; omega)
              | (apply Prod.Lex.right; apply Prod.Lex.right; omega))
       have ⟨e₂, he₂⟩ : ∃ e : T.Derivation _ _, e.isStronglyNormal = true := by
         rcases DB with _ | _ | G_and2 E₁ E₂ | _ | _ | _ | _ | _ | _ | _
         all_goals first
         | (simp [isStronglyNormal] at hD'_sn ⊢; exact ⟨_, hD'_sn.2.1⟩)
         | exact ⟨andE1 _ _, by simp_all [isStronglyNormal]⟩
         | (apply ih_d
            simp only [nodeCount, commutingSum, maximalFormulas] at h ⊢
            split_ifs <;> simp_all [maximalFormulas, commutingSum, nodeCount, isStronglyNormal] <;>
              ...
              | (apply Prod.Lex.right; apply Prod.Lex.right; omega)
              | (apply Prod.Lex.right; apply Prod.Lex.right; omega))
       -- D'' must not be orI1/orI2/orE for orE G_or D'' e₁ e₂ to be SN
       rcases D'' with _ | _ | _ | _ | _ | _ | _ | _ | _ | _ <;>
         simp_all [isStronglyNormal] <;>
         exact ⟨orE _ _ e₁ e₂, by simp_all [isStronglyNormal]⟩)
    -- impE case: andE1 G (impE ...) is SN when impE ... is SN
    | exact ⟨andE1 G _, by simp_all [isStronglyNormal]⟩
  | andE2 G D ihD =>
    simp only [isStronglyNormal] at h_sn
    have ⟨D', hD'_sn⟩ := ih_d _ _ D (by
      ...
      rcases D with _ | _ | _ | _ | _ | _ | _ | _ | _ | _ <;>
        ...
        | (apply Prod.Lex.right; apply Prod.Lex.right; omega)
        | (apply Prod.Lex.right; apply Prod.Lex.right; omega))
    rcases D' with _ | _ | G_and D₁ D₂ | _ | _ | _ | _ | G_or D'' DA DB | _ | _
    all_goals first
    | exact ⟨andE2 G _, by simp_all [isStronglyNormal]⟩
    | (rename_i D₁ D₂
       simp only [isStronglyNormal, Bool.and_eq_true] at hD'_sn
       exact ⟨D₂, hD'_sn.2⟩)
    | (rename_i D'' DA DB
       ...
       have ⟨e₁, he₁⟩ : ∃ e : T.Derivation _ _, e.isStronglyNormal = true := by
         ...
         all_goals first
         | (simp [isStronglyNormal] at hD'_sn ⊢; exact ⟨_, hD'_sn.1⟩)
         | exact ⟨andE2 _ _, by simp_all [isStronglyNormal]⟩
         | (apply ih_d
            simp only [nodeCount, commutingSum, maximalFormulas] at h ⊢
            split_ifs <;> simp_all [maximalFormulas, commutingSum, nodeCount, isStronglyNormal] <;>
              first
  ...
  | orI1 G D ihD =>
    ...
    have ⟨D', hD'⟩ := ih_d _ _ D (by
      simp only [nodeCount, commutingSum, maximalFormulas] at h ⊢
      apply Prod.Lex.right; apply Prod.Lex.right; omega)
    exact ⟨orI1 G D', by simp [isStronglyNormal, hD']⟩
  | orI2 G D ihD =>
    simp only [isStronglyNormal] at h_sn
    have ⟨D', hD'⟩ := ih_d _ _ D (by
      simp only [nodeCount, commutingSum, maximalFormulas] at h ⊢
      apply Prod.Lex.right; apply Prod.Lex.right; omega)
    exact ⟨orI2 G D', by simp [isStronglyNormal, hD']⟩
  | orE G D DA DB ihD ihDA ihDB =>
    simp only [isStronglyNormal] at h_sn
    -- Normalize D
    have ⟨D', hD'_sn⟩ := ih_d _ _ D (by
      simp only [nodeCount, commutingSum, maximalFormulas] at h ⊢
      rcases D with _ | _ | _ | _ | _ | _ | _ | _ | _ | _ <;>
        simp_all [maximalFormulas, commutingSum, nodeCount] <;>
        first
        | (apply Prod.Lex.left; first
           | exact Multiset.isDershowitzMannaLT_cons_add
           | exact Multiset.isDershowitzMannaLT_cons_add')
        | (apply Prod.Lex.right; apply Prod.Lex.right; omega)
        | (apply Prod.Lex.right; apply Prod.Lex.right; omega))
    -- Normalize DA and DB regardless
    have ⟨DA', hDA'_sn⟩ := ih_d _ _ DA (by
      simp only [nodeCount, commutingSum, maximalFormulas] at h ⊢
      rcases D with _ | _ | _ | _ | _ | _ | _ | _ | _ | _ <;>
        simp_all [maximalFormulas, commutingSum, nodeCount] <;>
        first
        ...
        | (apply Prod.Lex.right; apply Prod.Lex.right; omega)
        | (apply Prod.Lex.right; apply Prod.Lex.right; omega))
    have ⟨DB', hDB'_sn⟩ := ih_d _ _ DB (by
      simp only [nodeCount, commutingSum, maximalFormulas] at h ⊢
      rcases D with _ | _ | _ | _ | _ | _ | _ | _ | _ | _ <;>
        simp_all [maximalFormulas, commutingSum, nodeCount] <;>
        first
        ...
        | (apply Prod.Lex.right; apply Prod.Lex.right; omega)
        | (apply Prod.Lex.right; apply Prod.Lex.right; omega))
    -- Case-split on D' to determine SN of orE G D' DA' DB'
    rcases D' with _ | _ | _ | _ | _ | G_or D_inj | G_or D_inj | _ | _ | _
    -- D' = orI1: beta redex → DA'.subsOne D_inj; need DA'.sn ∧ D_inj.sn
    all_goals first
    -- For orI1/orI2: beta reduction to subsOne, need SN of argument and branch
    | (simp only [isStronglyNormal] at hD'_sn
       -- D' = orI1 _ D_inj with D_inj.sn
       -- Apply ih_d to get SN form of DA'.subsOne D_inj via normMeasure decrease
       apply ih_d
       simp only [normMeasure, maximalFormulas, commutingSum, nodeCount] at h ⊢
       have hmf_DA' : DA'.maximalFormulas = ∅ := maximalFormulas_sn_eq_zero _ hDA'_sn
       have hmf_Dinj : D_inj.maximalFormulas = ∅ := maximalFormulas_sn_eq_zero _ hD'_sn
       apply Prod.Lex.left
       simp only [maximalFormulas]
       rw [hmf_DA', hmf_Dinj]
       apply Multiset.isDershowitzMannaLT_all_lt
       intro y hy
       have hynew : y ∉ DA'.maximalFormulas := by simp [hmf_DA']
       rcases subsOne_new_redex_complexity_lt DA' D_inj hy hynew with h | h
       · simp [hmf_Dinj] at h
       · rw [h]; simp only [Proposition.complexity]; omega)
    -- orE _ D'' DA DB non-special: just return orE G D' DA' DB'
    | (rcases D' with _ | _ | _ | _ | _ | _ | _ | G_or D'' DA'' DB'' | _ | _ <;>
        simp_all [isStronglyNormal] <;>
        exact ⟨orE _ _ DA' DB', by simp_all [isStronglyNormal]⟩)
    -- ax/ass/andI/andE1/andE2/orI1(handled)/orI2(handled)/impI/impE: need orE SN
    | exact ⟨orE _ D' DA' DB', by simp_all [isStronglyNormal]⟩
  | impI G D ihD =>
    simp only [isStronglyNormal] at h_sn
    have ⟨D', hD'⟩ := ih_d _ _ D (by
      simp only [nodeCount, commutingSum, maximalFormulas] at h ⊢
      apply Prod.Lex.right; apply Prod.Lex.right; omega)
    exact ⟨impI G D', by simp [isStronglyNormal, hD']⟩
  | impE D E ihD ihE =>
    simp only [isStronglyNormal] at h_sn
    -- Get SN forms of D and E
    have ⟨D', hD'_sn⟩ := ih_d _ _ D (by
      simp only [nodeCount, commutingSum, maximalFormulas] at h ⊢
      rcases D with _ | _ | _ | _ | _ | _ | _ | _ | _ | _ <;>
        simp_all [maximalFormulas, commutingSum, nodeCount] <;>
    ...
    have ⟨E', hE'_sn⟩ := ih_d _ _ E (by
      ...
      rcases D with _ | _ | _ | _ | _ | _ | _ | _ | _ | _ <;>
        ...
        | (apply Prod.Lex.right; apply Prod.Lex.right; omega)
        | (apply Prod.Lex.right; apply Prod.Lex.right; omega))
    -- Case-split on D' to see if impE D' E' is SN
    rcases D' with _ | _ | _ | _ | _ | _ | _ | G_or D'' DA DB | G_imp D_body | _
    all_goals first
    -- impI case: beta redex, apply ih_d to D_body.subsOne E'
    | (simp only [isStronglyNormal] at hD'_sn
       apply ih_d
       simp only [maximalFormulas, commutingSum, nodeCount] at h ⊢
       have hmf_D' : D_body.maximalFormulas = ∅ := maximalFormulas_sn_eq_zero _ hD'_sn
       have hmf_E' : E'.maximalFormulas = ∅ := maximalFormulas_sn_eq_zero _ hE'_sn
       apply Prod.Lex.left
       simp only [maximalFormulas]
       rw [hmf_D', hmf_E']
       apply Multiset.isDershowitzMannaLT_all_lt
       intro y hy
       have hynew : y ∉ D_body.maximalFormulas := by simp [hmf_D']
       rcases subsOne_new_redex_complexity_lt D_body E' hy hynew with hE | hA
       · simp [hmf_E'] at hE
       · rw [hA]; simp only [Proposition.complexity]; omega)
    -- orE case: commuting conversion, need SN forms of pushed branches
    | (simp only [isStronglyNormal] at hD'_sn
       rename_i D'' DA DB
       -- Get SN form of impE DA (E'.weakCtx ...)
       have hE'_mf : E'.maximalFormulas = ∅ := maximalFormulas_sn_eq_zero _ hE'_sn
       have ⟨f₁, hf₁⟩ : ∃ f : T.Derivation _ _, f.isStronglyNormal = true := by
         rcases DA with _ | _ | G_and2 E₁ E₂ | _ | _ | _ | _ | _ | G_imp2 Db | _
         all_goals first
         | (simp only [isStronglyNormal] at hD'_sn
            apply ih_d
            simp only [maximalFormulas, commutingSum, nodeCount] at h ⊢
            have hmf_DA : DA.maximalFormulas = ∅ := maximalFormulas_sn_eq_zero _
                (by simp_all [isStronglyNormal])
            have hmf_D'' : D''.maximalFormulas = ∅ := maximalFormulas_sn_eq_zero _
                (by simp_all [isStronglyNormal])
            apply Prod.Lex.left
            simp only [maximalFormulas]
            rw [hmf_DA, hE'_mf]
            ...
            intro y hy
            have hynew : y ∉ DA.maximalFormulas := by simp [hmf_DA]
            rcases subsOne_new_redex_complexity_lt DA (E'.weakCtx (Finset.subset_insert _ _)) hy hynew with hE | hA
            · rw [maximalFormulas_weakCtx] at hE; simp [hE'_mf] at hE
            · rw [hA]; simp only [Proposition.complexity]; omega)
         | (exact ⟨impE _ _, by simp_all [isStronglyNormal, maximalFormulas_weakCtx]⟩)
       have ⟨f₂, hf₂⟩ : ∃ f : T.Derivation _ _, f.isStronglyNormal = true := by
         rcases DB with _ | _ | G_and2 E₁ E₂ | _ | _ | _ | _ | _ | G_imp2 Db | _
         all_goals first
         | (simp only [isStronglyNormal] at hD'_sn
            apply ih_d
            simp only [maximalFormulas, commutingSum, nodeCount] at h ⊢
            have hmf_DB : DB.maximalFormulas = ∅ := maximalFormulas_sn_eq_zero _
                (by simp_all [isStronglyNormal])
            apply Prod.Lex.left
            simp only [maximalFormulas]
            rw [hmf_DB, hE'_mf]
            apply Multiset.isDershowitzMannaLT_all_lt
            intro y hy
            have hynew : y ∉ DB.maximalFormulas := by simp [hmf_DB]
            rcases subsOne_new_redex_complexity_lt DB (E'.weakCtx (Finset.subset_insert _ _)) hy hynew with hE | hA
            · rw [maximalFormulas_weakCtx] at hE; simp [hE'_mf] at hE
            · rw [hA]; simp only [Proposition.complexity]; omega)
         | (exact ⟨impE _ _, by simp_all [isStronglyNormal, maximalFormulas_weakCtx]⟩)
       rcases D'' with _ | _ | _ | _ | _ | _ | _ | _ | _ | _ <;>
         simp_all [isStronglyNormal] <;>
         exact ⟨orE _ _ f₁ f₂, by simp_all [isStronglyNormal]⟩)
    -- All other cases: impE D' E' is SN
    | exact ⟨impE D' E', by simp_all [isStronglyNormal]⟩

/-! ## Main Subformula Property Theorem -/

/-- The main subformula property: every derivation has a strongly normal form satisfying
the subformula property.

Proved via `exists_stronglyNormal_form`: the well-founded normalization theorem shows
every derivation has a strongly normal form (using the Dershowitz–Manna termination
measure), and strongly normal derivations satisfy the subformula property.
([Prawitz1965], Ch. III–IV; [Troelstra-Schwichtenberg2000] §6.1.8/6.12.) -/
theorem Theory.Derivation.subformula_property {G : Ctx Atom} {A : Proposition Atom}
  ...
  obtain ⟨d', hd'_sn⟩ := d.exists_stronglyNormal_form
  exact ⟨d', hd'_sn, d'.subformula_property_of_isStronglyNormal hd'_sn⟩
