...
/-- **The corrected-gate KB5 rule's box-positive semantic soundness**, direct analogue of
`modalKb5BoxAllFull_soundIn` against `modalKb5BoxAllUniv`'s trigger-free dichotomy
(`modalKb5BoxAllUniv_mem`). The self-target arm (`x.label = 0`) now splits into two sub-cases on
whether the trigger itself is the root: `lbl = 0` discharges via `accReachableInv_kb5_root_refl`
exactly as the frozen rule did; `lbl ≠ 0` is the genuinely new case, discharged via
`reachable_imp_related_kb5_symm`. -/
...
Mirrors `modalStepBranchKb5'_preserves_accReachableInv`, with one addition: since the corrected
rule's self-target arm needs `(0 : WorldIndex) ∈ modalKnownWorlds b` threaded in
(`modalApplyOneKb5''Prop_knownWorlds_step`'s `h0`), the conclusion is strengthened to also
propagate `0 ∈ modalKnownWorlds b'` to every child branch `b'` -- trivial by
...
always known" invariant, given `accTargetsKnown`. Mirrors
`modalStepBranchKb5'_preserves_accReachableInv`. -/
lemma modalStepBranchKb5''_preserves_accReachableInv
    (b e : List (SignedFormula (Proposition Atom) WorldIndex)) (acc : Accessibility)
    (newBs newExps : List (List (SignedFormula (Proposition Atom) WorldIndex)))
    (newAcc : Accessibility)
...
Mirrors `modalStepBranchKb5'_preserves_satIn` exactly, substituting `modalApplyOneKb5''` for
`modalApplyOneKb5'` and the corrected-gate rule-soundness lemmas
(`modalKb5BoxAllUniv_soundIn`/`modalKb5DiaNegAllUniv_soundIn`) for their frozen `Full`
counterparts. No `h0` ("root known") threading is needed here: this lemma tracks semantic
