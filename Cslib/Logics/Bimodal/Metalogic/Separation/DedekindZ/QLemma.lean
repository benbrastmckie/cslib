...
def kPlus (q : Formula Atom) : Formula Atom :=
  Formula.neg (.untl (Formula.neg q) (Formula.neg .bot))

/-- K-(q) = not(S(top, not q)). -/
def kMinus (q : Formula Atom) : Formula Atom :=
  Formula.neg (.snce (Formula.neg q) (Formula.neg .bot))
...
def qZ (A B C : Formula Atom) : Formula Atom :=
  Formula.or (Formula.or B A) (Formula.neg (.snce (Formula.neg A) C))
...
theorem Q_lemma_Z_fwd (A B C : Formula Atom) (M : IntStructure Atom) (t0 t1 : ℤ)
    ...
    (hguard : ∀ z : ℤ, t0 < z → z < t1 →
      (intTruth M z C → intTruth M z (.untl B A)))
    (hinit : intTruth M t0 (.untl B A)) :
    ∀ z : ℤ, t0 < z → z < t1 → intTruth M z (qZ A B C) := by
  intro z hz0 hz1
  rw [qZ, int_truth_or, int_truth_or, int_truth_neg]
  by_cases hS : intTruth M z (.snce (Formula.neg A) C)
...
theorem Q_lemma_Z_bwd (A B C : Formula Atom) (M : IntStructure Atom) (t0 t1 : ℤ)
    ...
    (hend : intTruth M t1 A
          ∨ intTruth M t1 (Formula.and B (.untl B A))) :
    ∀ z : ℤ, t0 < z → z < t1 →
      (intTruth M z C → intTruth M z (.untl B A)) := by
  intro z hz0 hz1 hCz
...
def case3Alpha (a q A B : Formula Atom) : Formula Atom :=
  Formula.or a
    (Formula.and (Formula.and (Formula.neg q) (.snce q a))
...
def case3Rhs (a q A B : Formula Atom) : Formula Atom :=
  ...
  Formula.or (Formula.or
    (.snce q a)
    (Formula.and (.snce qz al)
      (Formula.or A (Formula.and B (.untl B A)))))
    (.snce q
...
theorem case3_equiv_Z_bwd (a q A B : Formula Atom) (M : IntStructure Atom) (t : ℤ)
    ...
    intTruth M t (.snce (Formula.or q (.untl B A)) a) := by
  ...
  · rcases (int_truth_or M t _ _).mp h12 with h1 | h2
    ...
    · have ⟨hSalpha, hABU⟩ := (int_truth_and M t _ _).mp h2
      ...
      · have hend_for_Q : intTruth M t A ∨ intTruth M t (Formula.and B (.untl B A)) := by
  ...
  · obtain ⟨u, hut, hevent_u, hq_guard⟩ := h3
    ...
    simp only [case3Alpha] at halpha_v
    rcases (int_truth_or M v _ _).mp halpha_v with ha_v | halpha2
    · have hend_u : intTruth M u A ∨ intTruth M u (Formula.and B (.untl B A)) :=
        Or.inl hA_u
...
theorem case3_equiv_Z_fwd (a q A B : Formula Atom) (M : IntStructure Atom) (t : ℤ)
  ...
  · push Not at hq_all
    ...
    · have hqU_f₀ := hguard f₀ hsf₀ hf₀t
      have hU_f₀ : intTruth M f₀ (.untl B A) := by
      ...
      have hSaq_f₀ : intTruth M f₀ (.snce q a) :=
        ⟨s, hsf₀, ha_s, hq_left⟩
      ...
      have hQ_on_interval : ∀ z, f₀ < z → z < t → intTruth M z (qZ A B (Formula.neg q)) := by
        ...
        · exact hU_f₀
      have hSalpha_t : intTruth M t (.snce (qZ A B (Formula.neg q)) (case3Alpha a q A B)) :=
      ...
      · rcases eq_or_lt_of_le hwt with rfl | hwt'
        ...
        · have hqw : intTruth M w q := hq_right w hf₀w hwt'
          have hqU_w : intTruth M w (Formula.or q (.untl B A)) :=
            (int_truth_or M w _ _).mpr (Or.inl hqw)
          have hSalpha_w : intTruth M w (.snce (qZ A B (Formula.neg q)) (case3Alpha a q A B)) :=
            ⟨f₀, hf₀w, halpha_f₀, fun z hz1 hz2 => hQ_on_interval z hz1 (lt_trans hz2 hwt')⟩
          have hevent_w : intTruth M w (Formula.and (Formula.and A (Formula.or q (.untl B A)))
               (.snce (qZ A B (Formula.neg q)) (case3Alpha a q A B))) :=
    ...
    · push Not at hq_right
      ...
      have hU_g : intTruth M g (.untl B A) := by
...
theorem case3_equiv_Z_general (a q A B : Formula Atom) :
    intEquiv (.snce (Formula.or q (.untl B A)) a) (case3Rhs a q A B) :=
