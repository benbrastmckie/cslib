/-
Refutation spike: is `tableau_complete`'s `hvalid` premise derivable from `IValid φ`?

`tableau_complete` (Scheme.lean) demands

  hvalid : ∀ (edges : IEdges) (b : IBranch Atom),
    @IForces Atom Nat (intAccessPreorder edges) (intExtractValuation b) (S.modelBot b) 0 φ

quantified over ARBITRARY `edges` and ARBITRARY `b`, with no hypothesis tying either to an
actual tableau run. `IValid φ` only supplies forcing for UPWARD-CLOSED valuations. This file
exhibits a concrete `(edges, b, φ)` with `IValid φ` true and the `hvalid` body FALSE, which
shows the DP-3/DP-4 goals are not merely hard but unprovable AS STATED.

Witness: φ = p → (q → p) (the `implyK` axiom shape, intuitionistically valid),
