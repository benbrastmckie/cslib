...
-- File-wide `open Classical` (not per-declaration) is used throughout for `byContradiction`/
-- `em`; replacing it with per-declaration `open scoped Classical in` is a real refactor
-- across every theorem below, deferred (not this pass's scope -- see the mandate against
-- altering proof terms/statements).
...
-- `simp ... at *` genuinely needs to simplify the induction hypotheses `hu`/`hs` together
-- with the goal in the `imp` case; a `simp only [...]` replacement needs `simp?`-verified
-- lemma sets per branch, deferred as real refactoring work (not this pass's scope).
...
theorem separated_imp_separable (φ : Formula Atom)
    (h : isSyntacticallySeparated φ = true) :
    isSeparable φ :=
...
theorem lemma_10_2_4_gen (a q A B : Formula Atom)
    ...
    isSeparable
      (.snce (Formula.or q (Formula.neg (.untl B A))) (Formula.and a (Formula.neg (.untl B A)))) :=
