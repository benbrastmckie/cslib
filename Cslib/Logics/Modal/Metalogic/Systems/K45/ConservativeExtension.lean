...
/-! # Modal K45 as a Conservative Extension of Classical Propositional Logic

This module proves that Modal K45 is a conservative extension of Classical Propositional Logic
(CPL): any propositional formula `φ` that is derivable in K45 (as `φ.toModal`) is already
derivable in CPL.

The proof uses the universal model `(Unit, fun _ _ => True, fun _ => v)`, which satisfies
all K45 frame conditions (transitivity and Euclideanness: both `fun _ _ _ _ _ => trivial`).
...
- `k45_conservative_extension`: If `φ.toModal` is K45-derivable then `φ` is CPL-derivable.
...
/-- **Modal K45 Conservative Extension over CPL**:

If the modal translation `φ.toModal` is derivable in K45, then `φ` is derivable
in Classical Propositional Logic.

Proof: construct the universal model `(Unit, fun _ _ => True, fun _ => v)`.
Transitivity and Euclideanness hold vacuously. K45 soundness gives satisfaction
...
theorem k45_conservative_extension {Atom : Type*} {φ : PL.Proposition Atom}
    (h : Derivable (@K45Axiom Atom) φ.toModal) :
  ...
  exact (modal_satisfies_toModal_iff_evaluate m () φ).mp
    (k45_soundness d m (fun _ _ _ _ _ => trivial) (fun _ _ _ _ _ => trivial)
      () (fun _ h => nomatch h))
