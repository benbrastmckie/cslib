...
structure EnrichedEvent (A : Set (Formula Atom)) (guard event : Formula Atom)
...
private noncomputable def iteratedEnrichment {A : Set (Formula Atom)}
  ...
  | [], _, event, hUntl =>
    EnrichedEvent.mk event hUntl (identity' event) (fun _ h => by simp at h)
...
structure EnrichedEventSince (C : Set (Formula Atom)) (guard event : Formula Atom)
...
private def lemma27Seed (A B _C : Set (Formula Atom)) (xi eta : Formula Atom) :
    Set (Formula Atom) :=
...
private theorem lemma_2_7_seed_consistent {A B C : Set (Formula Atom)}
  ...
  have h_neg_until_exists : ∃ beta0 ∈ B, ∃ gamma0 ∈ C,
    ...
    have h_rsince : burgessRSetSince C (deductiveClosure ({xi} ∪ B)) A := by
      ...
      · have h_burgessR_ext : burgessR A (Formula.and beta_w xi) C :=
        ...
        have h_snce_ext :=
          burgessR_implies_burgessRSince h_mcs_A h_mcs_C h_burgessR_ext alpha halpha
  ...
  suffices h_key : ∀ (b : Formula Atom) (hb : b ∈ B)
      (h_b_beta0 : DerivationTree FrameClass.Base [] (b.imp beta0))
      (γ_hat : Formula Atom) (hγ : γ_hat ∈ C)
      (h_γ_gamma0 : DerivationTree FrameClass.Base [] (γ_hat.imp gamma0))
      ...
      Σ' (event : Formula Atom),
        ...
        (∀ α ∈ alpha_list, DerivationTree FrameClass.Base []
    ...
    have h_event_implies_L : ∀ φ ∈ L, DerivationTree FrameClass.Base [event] φ := by
      ...
      · have h_φ_in_raw : φ ∈ b_list_raw :=
          l27_collect_guards_mem_of_B h_B_dcs xi eta L hL φ hφ h_B_case
        have h_φ_in_b : φ ∈ b_list := List.mem_cons.mpr (Or.inr h_φ_in_raw)
        have h_b_to_φ : DerivationTree FrameClass.Base [] (b.imp φ) :=
          listConjImpliesElem b_list φ h_φ_in_b
      ...
      · by_cases h_eta : φ = eta
        ...
        · by_cases h_snce5 : ∃ β' ∈ B, ∃ α ∈ A, φ = Formula.snce (Formula.and β' xi) α
          · let β' := Classical.choose h_snce5
            ...
            · have h_in_raw := l27_collect_guards_mem_of_B h_B_dcs xi eta L hL
              ...
              have h_b_imp :
                  DerivationTree FrameClass.Base [] (b.imp (Formula.snce (Formula.and β' xi) α')) :=
              ...
              exact DerivationTree.modus_ponens _ _ _
                ...
                (DerivationTree.assumption _ _ (by exact List.mem_singleton.mpr rfl))
            · have h_α'_in_a :=
                @l27_a_event_list_α_mem_xi _ A B C xi eta L hL β' α' h_φ_eq_snce5 hβ' hα'
              have h_ev_snce_α' := h_ev_snce α' h_α'_in_a
              have h_β'_in_raw :=
                l27_collect_guards_mem_of_snce_xi h_B_dcs xi eta L hL β' α' h_φ_eq_snce5 hβ' hα'
                  h_snce5_B
              have h_β'_in_b : β' ∈ b_list := List.mem_cons.mpr (Or.inr h_β'_in_raw)
              have h_b_to_β' : DerivationTree FrameClass.Base [] (b.imp β') :=
                listConjImpliesElem b_list β' h_β'_in_b
              have h_bχ_to_β'xi : DerivationTree FrameClass.Base []
                  ((Formula.and b χ_gen).imp (Formula.and β' xi)) := by
                have h1 : DerivationTree FrameClass.Base [] _ := impTrans (lceImp b χ_gen) h_b_to_β'
                have h2 : DerivationTree FrameClass.Base [] _ :=
                  impTrans (rceImp b χ_gen) (lceImp xi (Formula.untl xi eta))
                exact combineImpConj h1 h2
              have h_mono :=
                snceLeftMonoDeriv (Formula.and b χ_gen) α' (Formula.and β' xi) h_bχ_to_β'xi
  ...
  have h_guard_to_b0xi : DerivationTree FrameClass.Base []
      ((Formula.and φ_gen χ_gen).imp (Formula.and beta0 xi)) := by
    have h1 : DerivationTree FrameClass.Base [] _ :=
...
theorem lemma_2_7 {A B C : Set (Formula Atom)}
    ...
    ∃ B' D B'' : Set (Formula Atom),
      ...
      xi ∈ B' := by
  have h_seed_cons :=
    lemma_2_7_seed_consistent h_mcs_A h_mcs_C h_r3m h_B_dcs h_gc xi eta h_until h_xi_not_B
...
private theorem lemma_2_8_seed_consistent {A B C : Set (Formula Atom)}
  ...
  have h_γ'_to_neg_chi :
      DerivationTree FrameClass.Base [] (γ'.imp (Formula.and xi (Formula.untl xi eta)).neg) :=
    impTrans (demorganDisjNegForward eta (Formula.and xi (Formula.untl xi eta)))
      (rceImp eta.neg (Formula.and xi (Formula.untl xi eta)).neg)
  have h_bx5_xe := self_accum_until_mcs h_mcs_A xi eta h_until
  suffices h_key : ∀ (b : Formula Atom) (hb : b ∈ B)
      (γ_hat : Formula Atom) (hγ : γ_hat ∈ C)
      (h_γ_to_γ' : DerivationTree FrameClass.Base [] (γ_hat.imp γ'))
      (alpha_list : List (Formula Atom)) (h_alphas : ∀ α ∈ alpha_list, α ∈ A),
      Σ' (event : Formula Atom),
        (𝐅event) ∈ A ×'
        DerivationTree FrameClass.Base [] (event.imp b) ×'
    ...
    have h_event_implies_L : ∀ φ ∈ L, DerivationTree FrameClass.Base [event] φ := by
      ...
      · have h_φ_in_raw : φ ∈ b_list_raw :=
        ...
        have h_b_to_φ : DerivationTree FrameClass.Base [] (b.imp φ) :=
          listConjImpliesElem b_list_full φ h_φ_in_b
        have h_ev_to_φ : DerivationTree FrameClass.Base [] (event.imp φ) := impTrans h_ev_b h_b_to_φ
        exact DerivationTree.modus_ponens _ _ _
          (DerivationTree.weakening [] _ _ h_ev_to_φ (List.nil_subset _))
          (DerivationTree.assumption _ _ (by exact List.mem_singleton.mpr rfl))
        ...
        · by_cases h_snce5 : ∃ β' ∈ B, ∃ α ∈ A, φ = Formula.snce (Formula.and β' xi) α
          · let β' := Classical.choose h_snce5
            ...
            · have h_α'_in_a :=
              ...
              have h_b_to_β' : DerivationTree FrameClass.Base [] (b.imp β') :=
                listConjImpliesElem b_list_full β' h_β'_in_b
              have h_bχ_to_β'xi : DerivationTree FrameClass.Base []
                  ((Formula.and b χ_gen).imp (Formula.and β' xi)) := by
                have h1 : DerivationTree FrameClass.Base [] _ := impTrans (lceImp b χ_gen) h_b_to_β'
                have h2 : DerivationTree FrameClass.Base [] _ :=
  ...
  have h_D3_gen : Formula.untl (Formula.and φ_gen χ_gen) (Formula.and φ_gen eta) ∈ A := by
    ...
    · exfalso
      have h_event_to_bot :
          DerivationTree FrameClass.Base [] ((Formula.and γ_hat eta).imp Formula.bot) := by
        ...
        have d1 : DerivationTree FrameClass.Base [PConj] eta.neg :=
      ...
      have h_G_top : Formula.allFuture (Formula.bot.imp Formula.bot) ∈ A :=
        theoremInMcs h_mcs_A
          (DerivationTree.temporal_necessitation _ (identity' (Formula.bot : Formula Atom)))
      exact someFuture_allFuture_neg_absurd h_mcs_A Formula.bot h_F_bot h_G_top
    · exfalso
      have h_event_to_bot :
          DerivationTree FrameClass.Base [] ((Formula.and γ_hat χ_gen).imp Formula.bot) := by
        ...
        have d1 : DerivationTree FrameClass.Base [PConj] χ_gen.neg :=
          DerivationTree.modus_ponens _ _ _
          (DerivationTree.weakening [] _ _ h1 (List.nil_subset _))
          (DerivationTree.assumption _ PConj (by simp))
