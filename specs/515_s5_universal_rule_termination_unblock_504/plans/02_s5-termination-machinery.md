...
- [ ] Prove `modalStepBranchS5g_preserves_keysTotal` — every newly-known world gets a recorded key; consumes `accKnown` (P3) feeding `modalApplyOne_knownWorlds_step` (`FmpMeasure.lean:2042`) for the label-provenance dichotomy. *(deviation: NOT STARTED this dispatch, same budget reason as `keyLowerBd`. Recommended to reuse `modalApplyOneS5_knownWorlds_step` (landed Phase 4, cycle 4) + `accKnown` directly -- see handoff.)*
