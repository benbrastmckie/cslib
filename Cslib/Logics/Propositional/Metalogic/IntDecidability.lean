...
status, and the payoff is low (see the `int_fin_truth_lemma` docstring below for the full
disposition). See `int_fin_truth_lemma` vs the parametric `truthLemma` in
