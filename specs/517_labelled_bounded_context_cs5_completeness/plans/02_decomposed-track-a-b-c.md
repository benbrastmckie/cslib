...
| C3 (NEXT) | (6.8): `(◇Conj(W) ⊃ □Tele(W,B)) ⊃ □Tele(W,B)`, induction on W, **axiom 5 (requires A1)** | sorry-free | MED — deferred this dispatch per H8 phase sizing (C2's base-case scoping correction consumed the budget; do not rush C3's own base case without the same countermodel-style sanity check first). |
