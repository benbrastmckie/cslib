...
**Goals**:
- Extend `S5LoopInv` from four fields to a twelve-field shape (ten mirroring the landed `S4LoopInv`, `LoopChecking.lean:1127`, plus two S5-specific additions, `keysKnown`/`worldsContiguous`), naming each new field and what it guarantees.
...
- Prove the corrected preservation lemmas over the twelve-field invariant and the pigeonhole `#worlds ≤ 2^(2·|Sf|)` bound under the corrected guard.
...
### Phase 3: Keys-aware guard redesign + extended twelve-field `S5LoopInv` [COMPLETED]

**Goal**: Re-architect the termination bundle to defeat both v1 blocker gaps. Redesign the minting guard to compare the prospective birth content against the threaded **stored keys** (not live relevant sets), and extend `S5LoopInv` from four fields to a twelve-field shape (ten mirroring the landed `S4LoopInv`, `LoopChecking.lean:1127`, plus two S5-specific additions -- `keysKnown`/`worldsContiguous`, both landed as deviations, see task list below). **This is the crux structural fix; it supersedes the v1 four-field structure and the live-set minting decision.** Budget generously.
...
**Files to modify**:
- `Cslib/Logics/Modal/Tableau/S5Simplification.lean` — `blockingWorldS5Keyed` (+ `_none_fresh`), extended twelve-field `S5LoopInv`, updated `modalStepBranchS5gKeyed`
...
### Phase 4: Generic-field preservation lemmas [COMPLETED]
...
**Tasks**:
- [x] Prove `modalStepBranchS5g_preserves_bClosure` -- landed sorry-free this dispatch (cycle 4), unblocking the prior cycle-2/3 stopping point. *(deviation: required adding a 12th `S5LoopInv` field `worldsContiguous : modalMaxWorld b + 1 = (modalKnownWorlds b).length` (exactly as handoff `02_phase4-bclosure-contiguity-gap.md` recommended) plus its own preservation lemma `modalStepBranchS5g_preserves_worldsContiguous`, and a substantial new "`bClosure` support" layer of local re-derivations mirroring `FmpMeasure.lean`'s `modalApplyOne_outputs_subset`/`boxProps_outputs_subset`/`diaNegProps_outputs_subset`/`modalApplyOne_diamondPos_outputs_subset`/`modalApplyOne_boxNeg_outputs_subset` machinery (all `private` upstream, hence unavailable cross-file), swapped to `modalUniverseS5`/`modalWorldBoundS5`: `modalSubfmls_trans_S5`, `mem_modalUniverseS5_of`/`_of'`, `modalUniverseS5_mem_formula`/`_mem_label`, `mem_boxPositivesOf_S5`, `boxProps_outputs_subset_S5`, `diaNegProps_outputs_subset_S5`, `modalApplyOne_diamondPos_outputs_subset_S5`, `modalApplyOne_boxNeg_outputs_subset_S5`, the K-level top dispatch `modalApplyOne_outputs_subset_S5`, and finally `modalApplyOneS5_outputs_subset` (the S5-merge-aware top dispatch `bClosure`'s preservation consumes directly). Also added `modalApplyOneS5_knownWorlds_step` (S5 analogue of `modalApplyOne_knownWorlds_step`) as a byproduct, used by `worldsContiguous`'s own preservation. See handoff `02_phase4-bclosure-contiguity-gap.md` for the exact gap this closes.)*
- [x] Prove `modalStepBranchS5g_preserves_eNodup` -- landed sorry-free (cycle 2), direct case analysis on `modalStepBranchS5gKeyed`'s three-way dispatch (see `modalStepBranchS5gKeyed_expanded_shape` private helper).
- [x] Prove `modalStepBranchS5g_preserves_eClosure` -- landed sorry-free this dispatch (cycle 4). *(deviation: much simpler than `bClosure` once stated as a hypothesis-consuming lemma -- the only formula ever appended to `e` is the trigger `sf` itself, and `sf ∈ b` with the ambient `bClosure` hypothesis on `b` gives `sf ∈ modalUniverseS5 φ₀` directly; no mint-case label-bound/contiguity reasoning needed at all.)*
- [x] Prove `modalStepBranchS5g_preserves_accFresh` (reuse `accFreshInv` monotonicity) and `_preserves_accKnown` (reuse `accTargetsKnown` step lemmas; new/loop-back edges target known worlds) -- both landed sorry-free (cycle 2), recovered from stashed WIP and fixed to build.
- [x] Prove `modalStepBranchS5g_preserves_outDegEq` -- landed sorry-free (cycle 2). *(deviation: the stashed draft's final `rw [modalApplyOne_outDeg_step ..., ← modalApplyOneS5_eshape_eq ...]` failed with a "did not find pattern" error caused by Lean's per-declaration match-auxiliary-function identity -- two syntactically identical `match ... with` expressions in different lemma statements compile to distinct matcher constants, which `rw`'s kabstract cannot unify even though `exact`/term-mode elaboration can via defeq. Fixed by bridging the two lemmas' matches through a `have hfull := (modalApplyOne_outDeg_step ...).trans (congrArg (fun l => ...) (modalApplyOneS5_eshape_eq ...).symm)` term composition (which type-checks via `isDefEq`, not syntactic rw), then `rw [hfull]` and case-splitting cleanly. Also required an `omit [Hashable Atom] in` before the lemma to clear a new `unusedSectionVars` warning caught by `lake shake`.)*
- [x] *(deviation: added task, cycle 4)* Prove `modalStepBranchS5g_preserves_worldsContiguous` for the new 12th `S5LoopInv` field -- landed sorry-free, case analysis via `modalStepBranchS5gKeyed_acc_shape`: the non-mint case appends formulas at already-known labels (`modalMaxWorld`/known-world COUNT both unchanged, via new local re-derivations `modalMaxWorld_append_eq_of_forall_le_S5`/`modalKnownWorlds_length_append_of_known_S5`); the mint case appends a nonempty list all at exactly `modalNextWorld b` (both increment by exactly 1, via `modalMaxWorld_append_single_S5`/`modalKnownWorlds_length_append_single_S5`).

**Timing**: 2 hours (actual: ~2 hours cycle 2 + ~1 additional dispatch cycle 4 for the contiguity field + `bClosure`/`eClosure`)

**Depends on**: 3

**Files to modify**:
- `Cslib/Logics/Modal/Tableau/S5Simplification.lean` — six `modalStepBranchS5g_preserves_*` generic-field lemmas, plus the new `worldsContiguous` field/lemma and its supporting `bClosure`-closure infrastructure

**Verification**: full CI pipeline green (cycle 4: `lake build` 3239/3239, `lake exe checkInitImports`, `lake exe lint-style`, `lake lint` -- 0 new warnings, the 1 pre-existing unrelated `PrimeExclusion.lean` `unusedArguments` error untouched, `lake test` exit 0, `lake shake` -- 0 new import-minimization suggestions); all six generic-field preservation lemmas plus `worldsContiguous` close sorry-free. Zero `sorry`, zero new axioms in `S5Simplification.lean` (`lean_verify` on `modalStepBranchS5g_preserves_bClosure`/`_worldsContiguous`: only `propext`/`Classical.choice`/`Quot.sound`).

---

### Phase 5: Birth-key preservation lemmas (the `keysDistinct` crux) [COMPLETED]
...
- [x] Prove `modalStepBranchS5g_preserves_keysInUniverse` -- landed sorry-free this dispatch (cycle 4), on the first attempt. New keys `⊆ signedSubfmls φ₀`; consumes `bClosure` for the subformula-closure of the inserted witness `(s,φ)` pair, exactly mirroring `bClosure`'s own mint-case witness argument (`modalSubfmls_trans_S5`); the `successorBirthContentS5`-filtered remainder is trivially `⊆ signedSubfmls φ₀` via `Finset.filter_subset`. Confirms the handoff's prediction that this was the most tractable of the three remaining fields.
- [x] Prove `modalStepBranchS5g_preserves_keysTotal` — LANDED sorry-free (cycle 6). Exactly per handoff 04's recipe: (1) re-added the two cycle-5 standalone lemmas verbatim (`modalApplyOne_snd_eq_acc_of_not_mint_shape_S5`, `modalApplyOneS5_snd_eq_acc_of_not_mint_shape`, after `modalApplyOneS5_snd_eq`); (2) authored the combined helper `modalStepBranchS5gKeyed_keys_full_shape` giving `newBs`-shape AND `newKeys`-shape from ONE case split on the SAME `sf` -- compiled on the FIRST attempt by mirroring `_keys_shape`'s landed non-destructured `split at hsf <;> repeat' split at hsf` skeleton (NOT the cycle-5 buggy destructure-and-rewrite route; the split-generated shape hypotheses + `by assumption` witnesses avoid all three diagnosed bugs, and the default-arm no-match hypotheses feed `modalApplyOneS5_snd_eq_acc_of_not_mint_shape`'s `hns` via `rintro`+`simp_all`); (3) `keysTotal` itself: blocked case direct; wildcard case selects `modalApplyOneS5_knownWorlds_step`'s non-mint disjunct via the helper's `(modalApplyOneS5 sf b acc).snd = acc` fact + a new micro-helper `addEdge_ne_self_S5` (cons-length argument); mint case: old worlds keep old keys (`keys ⊆ newKeys`), the fresh world `modalNextWorld b` gets the same-event appended key.
- [x] Prove `modalStepBranchS5g_preserves_keyLowerBd` — LANDED sorry-free (cycle 6, same dispatch as `keysTotal`). Old keys: `relevantSetFinset_mono` under branch prepend, exactly as planned. Mint case per handoff 03's recipe: named the two mint-output groups as private defs `mintBoxPropsS5`/`mintDiaNegPropsS5` (single shared expression, avoiding cross-lemma matcher unification), authored the "introduction"-direction lemmas `boxPositivesOf_intro_S5` + `mintBoxPropsS5_mem_intro`/`mintDiaNegPropsS5_mem_intro` (the freshly-emitted-or-already-on-`b` dichotomy encoded by the dedup guard), full mint-arm characterizations `modalApplyOne_boxNeg_eq_S5`/`modalApplyOne_diamondPos_eq_S5` (strengthening `modalApplyOne_boxNeg_witness`'s existential `rest` to the concrete tail), and the sign-generic correspondence `successorBirthContentS5_subset_relevantSetFinset_mint` placing every birth-content pair on the minted branch at `modalNextWorld b`. Witness-pair `signedSubfmls` membership from `bClosure` exactly as in `keysInUniverse`.

**Timing**: 2.5 hours (actual: ~1 dispatch cycle for `keysDistinct` + `keysInUniverse`; `keyLowerBd`/`keysTotal` deferred)

**Depends on**: 4

**Files to modify**:
- `Cslib/Logics/Modal/Tableau/S5Simplification.lean` — four `modalStepBranchS5g_preserves_key*` lemmas (two landed: `keysDistinct`, `keysInUniverse`)

**Verification**: full CI pipeline green (cycle 4: `lake build` 3239/3239, `lake exe checkInitImports`, `lake exe lint-style`, `lake lint` 0 new warnings, `lake test` exit 0, `lake shake` 0 new suggestions for `S5Simplification.lean`); `modalStepBranchS5g_preserves_keysDistinct`/`_preserves_keysInUniverse` close sorry-free (`lean_verify`: only `propext`/`Classical.choice`/`Quot.sound`). **Cycle 6 (Phase 5 COMPLETE)**: full CI re-run green after `keysTotal`+`keyLowerBd` landed (`lake build` 3239/3239; `checkInitImports` clean; `lint-style` clean; `lake lint` only the 1 pre-existing unrelated `PrimeExclusion.lean` error; `lake test` exit 0; `lake shake` 0 suggestions for `S5Simplification.lean` -- the Temporal/* suggestions it prints belong to other tasks' committed files); `lean_verify` on `_preserves_keysTotal`/`_preserves_keyLowerBd`: only `propext`/`Classical.choice`/`Quot.sound`. Note: a pre-existing `sorry` warning in `Cslib/Logics/Propositional/Tableau/Minimal/Completeness.lean:118` (committed by task 317, commit `19c68791`) is outside this task's scope and files. **With Phase 5 complete, the full termination chain (Phases 3-6) is closed.**

**Blocked-branch**: neither `keysDistinct` nor `keysInUniverse` resisted -- both closed on the first real attempt once their respective supporting infrastructure (`modalStepBranchS5gKeyed_keys_shape`; `bClosure`/`modalSubfmls_trans_S5`) was in place, confirming the keys-aware guard design from Phase 3 and the `bClosure`-support layer from Phase 4 both work exactly as intended. `keyLowerBd`/`keysTotal` are `[NOT STARTED]` (not `[BLOCKED]`) pending the next dispatch's budget; see handoff `03_phase5-keysdistinct-landed-keylowerbd-scope.md` for the concrete next-step plan (also update its "Recommended next dispatch order" -- `keysInUniverse` is now DONE, so start with `keysTotal` next). No `sorry`, no weakening of `keysDistinct` to saturated-worlds-only (the 511-documented false shortcut).

**Cycle 5 attempt (keysTotal, NOT landed, reverted)**: spent the bulk of the cycle building a combined shape helper (`modalStepBranchS5gKeyed_keys_full_shape`, private) meant to give BOTH the `newBs`-shape (as `_acc_shape` does) and the `newKeys`-shape (as `_keys_shape` does) from a SINGLE case split on the SAME triggering `sf`, needed because `_acc_shape` and `_keys_shape` each independently `obtain` their own existential `sf` witness from `hstep`, so composing them in a caller cannot be shown to share the same `sf` without re-deriving both facts from one case split. Two small, genuinely reusable lemmas DID land and compile standalone (verified via `lake build`, no errors) before the combined helper's own construction went wrong -- worth re-adding verbatim in the next dispatch:
  - `modalApplyOne_snd_eq_acc_of_not_mint_shape_S5` (private, K-level): `(modalApplyOne sf b acc).snd = acc` given `¬(sf.sign=.neg ∧ ∃φ,sf.formula=.box φ) ∧ ¬(sf.sign=.pos ∧ ∃φ,sf.formula=.diamond φ)` -- i.e. K's own rule leaves `acc` unchanged outside its two minting shapes. Proved by `unfold modalApplyOne; by_cases hpa : (tryAllPropRules ...).isApplicable; · simp [hpa]; · rw [if_neg hpa]; obtain ⟨s,ff,l⟩ := sf; rcases s with _|_ <;> rcases ff with _|_|⟨a,c⟩|⟨x,y⟩|⟨x,y⟩|φ|φ` then per-branch `rfl` (5 trivial shapes), `dsimp only; split <;> rfl` (the propagation shape for that sign), `exact absurd ⟨rfl,φ,rfl⟩ h1_or_h2` (the excluded minting shape for that sign).
  - `modalApplyOneS5_snd_eq_acc_of_not_mint_shape` (private, S5-level wrapper): `rw [modalApplyOneS5_snd_eq]; exact modalApplyOne_snd_eq_acc_of_not_mint_shape_S5 sf b acc hns` -- lifts the above via the ALREADY-LANDED `modalApplyOneS5_snd_eq` (line ~347).
  Both inserted cleanly right after `modalApplyOneS5_snd_eq` and built with zero errors/new warnings in isolation.
  The combined shape helper itself hit THREE distinct, now-diagnosed bugs worth avoiding next time: (1) pre-establishing the shape witness via `rcases hshape with ⟨hsign,ψ,hform⟩` OUTSIDE the split, then `rw [hsign,hform] at hsf`, hits a HYGIENE mismatch -- `modalStepBranchS5gKeyed`'s own match arm uses `φ` as its bound pattern name, and rewriting a pre-supplied witness into the match discriminant causes Lean to rename the match's internal binder to an inaccessible `φ✝`, so the later existential-witness proof (needing literally `ψ`) fails to unify with `hsf`'s post-split `φ✝`-typed component; (2) `Sign`'s constructors are declared `pos` THEN `neg` (`Foundations/Logic/Tableau/Sign.lean:46-50`), so `rcases s with _ | _`'s FIRST bullet is `Sign.pos` (relevant to the pos-diamond shape), NOT `Sign.neg` as the natural reading-order might suggest -- assigning the neg-box handling to the first bullet silently mismatches every case tag; (3) the wildcard (non-minting-shape) sub-case's `hsf` does NOT auto-reduce past the RESULT-level `match (modalApplyOneS5 sf b acc).fst with ...` just because `sf`'s OWN sign/formula are concrete -- a `split at hsf <;> repeat' split at hsf` (or equivalent) is STILL required before `injection hsf` will accept the hypothesis, exactly as the special (minting) branches already do; skipping it produces "equality of constructor applications expected"/"too many identifiers" errors uniformly across every wildcard case. Fixing (1)+(2)+(3) together (unify the split path for special AND wildcard cases; use `_`/`by assumption` unification for existential witnesses rather than pre-supplied names; get `Sign`'s constructor order right) should make the combined helper (or an equivalent, more surgical `keysTotal`-only proof) tractable within a focused dispatch. Given the orchestrator's instruction to stop pushing on new proof obligations and land at a green checkpoint, the broken combined-helper draft was reverted (`git checkout`) rather than committed or left with a `sorry`; `S5Simplification.lean` is back to the exact CI-green state of commit `fcbe121c`.
...
### Phase 7: Soundness bridge `modalTableauS5_sound` [COMPLETED]
...
- [x] Assemble `modalTableauS5_sound` — LANDED sorry-free this dispatch (cycle 8). Chose Strategy 2 (bespoke copy, per handoff `06_*.md`'s recommendation) over extracting the K crux's ~490-line "not shape" branch into a shared generic lemma, to avoid any regression risk to the frozen K/T/B/S4 chain. Authored three new bespoke S5-specialized declarations in `FrameSoundness.lean`: (1) `S5SoundInv` (bundles `accFreshInv ∧ accReachableInv ∧ accTargetsKnown` into one `Prop`, threaded via a single `List.Forall₂`, mirroring how the generic fuel induction threads `accFreshInv` alone); (2) `modalStepBranchS5_preserves_satIn` (bespoke copy of `modalStepBranchGen_preserves_satIn`, specialized to `apply := modalApplyOneS5`/`FC := s5FC` with an added `hreach : accReachableInv b acc` parameter — the box-pos/dia-neg branch swaps in the landed `modalS5BoxAll_soundIn`/`modalS5DiaNegAll_soundIn` in place of the generic `hBoxPos`/`hDiaNeg` calls; every other shape is byte-identical to the K crux's own "not shape" branch, ported via `modalApplyOneS5_eq_of_not_boxPos_diaNeg` in place of `hAgree` — confirmed no other bare `apply`/`FC` occurrences exist in that ~400-line branch besides one `negImp_alpha_preserved_gen FC → s5FC` substitution); (3) `modalExpandBranchesS5_closed_unsatIn` (bespoke copy of `modalExpandBranchesGen_closed_unsatIn`, fixed to `modalApplyOneS5`/`s5FC`, threading `S5SoundInv` and combining all three step-preservation lemmas at each step — `modalStepBranch_preserves_accFreshInv_gen` and `modalStepBranch_preserves_accTargetsKnown_gen` (both already-generic, reused directly at `apply := modalApplyOneS5` via the landed `modalApplyOneS5_fresh_local`) plus the landed `modalStepBranchS5_preserves_accReachableInv`). `modalTableauS5_sound` itself mirrors `modalTableauT_sound` (`FrameCompleteness.lean:1182`) exactly. Zero edits to any pre-existing K/T/B/S4 declaration — confirmed zero-regression by the full `lake build`/`lake test` re-run.

**Timing**: 2 hours (cycle 7, four sub-tasks) + 1 dispatch cycle 8 for the final assembly

**Depends on**: 1 (landed)

**Files to modify**:
- `Cslib/Logics/Modal/Tableau/FrameSoundness.lean` — landed: `modalApplyOneS5_fresh_local`, `accReachableInv` (+`_initial`), `modalStepBranchS5_preserves_accReachableInv`, `reachable_imp_related_s5`, `accReachableInv_related_s5`, `modalS5BoxAll_soundIn`, `modalS5DiaNegAll_soundIn`, `S5SoundInv`, `modalStepBranchS5_preserves_satIn`, `modalExpandBranchesS5_closed_unsatIn`, `modalTableauS5_sound`.

**Verification**: full CI pipeline green (cycle 8: `lake build` 3239/3239; `checkInitImports` clean; `lake exe lint-style` clean; `lake lint` only the 1 pre-existing unrelated `PrimeExclusion.lean` `unusedArguments` error; `lake test` exit 0; `lake shake` — no NEW suggestions for `FrameSoundness.lean` introduced by this dispatch, the one flagged `remove import Mathlib.Data.List.Forall2` is a pre-existing redundant-transitive-import fact independent of this dispatch's additions since `Soundness.lean` (already `public import`ed by this file) also directly imports it and `List.Forall₂` was already used extensively by the pre-existing K/T crux before this dispatch); `lean_verify` on `modalTableauS5_sound`/`modalStepBranchS5_preserves_satIn`/`modalExpandBranchesS5_closed_unsatIn`: only `propext`/`Classical.choice`/`Quot.sound`. Zero `sorry`, zero new axioms.

**Phase 7 is now fully COMPLETE.** The full S5 soundness bridge (`modalTableauS5_sound`) lands sorry-free, independent of the termination chain (Phases 3-6, also complete).
...
- **P3**: `blockingWorldS5Keyed` (+ `_none_fresh`), extended twelve-field `S5LoopInv`, updated `modalStepBranchS5gKeyed` (`S5Simplification.lean`).
- **P4**: six `modalStepBranchS5g_preserves_*` generic-field lemmas plus `worldsContiguous` (the 12th `S5LoopInv` field) and its own preservation lemma, plus new `bClosure`-support universe-membership infrastructure (`S5Simplification.lean`).
