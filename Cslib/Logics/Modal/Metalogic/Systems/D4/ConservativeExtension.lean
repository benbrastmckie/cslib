...
/-! # Modal D4 as a Conservative Extension of Classical Propositional Logic

This module proves that Modal D4 is a conservative extension of Classical Propositional Logic
(CPL): any propositional formula `φ` that is derivable in D4 (as `φ.toModal`) is already
derivable in CPL.

The proof uses the universal model `(Unit, fun _ _ => True, fun _ => v)`, which satisfies
all D4 frame conditions. Seriality holds via `fun w => ⟨w, trivial⟩` and transitivity
...
- `d4_conservative_extension`: If `φ.toModal` is D4-derivable then `φ` is CPL-derivable.
...
/-- **Modal D4 Conservative Extension over CPL**:

If the modal translation `φ.toModal` is derivable in D4, then `φ` is derivable
in Classical Propositional Logic.

Proof: construct the universal model `(Unit, fun _ _ => True, fun _ => v)`.
Seriality holds via `fun w => ⟨w, trivial⟩` and transitivity holds vacuously.
D4 soundness gives satisfaction of `φ.toModal` at the single world; the bridge lemma
converts this to `Evaluate v φ`; CPL completeness then gives CPL derivability. -/
theorem d4_conservative_extension {Atom : Type*} {φ : PL.Proposition Atom}
    (h : Derivable (@D4Axiom Atom) φ.toModal) :
  ...
  exact (modal_satisfies_toModal_iff_evaluate m () φ).mp
    (d4_soundness d m ⟨fun w => ⟨w, trivial⟩⟩ (fun _ _ _ _ _ => trivial)
      () (fun _ h => nomatch h))
