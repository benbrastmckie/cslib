...
/-! # Labelled Syntax (Simpson's Labelled Natural Deduction, Task 517 Phase 1)
...
prerequisite for the `N_IK(𝒯)` deduction system (`Deduction.lean`, Phase 2) and, downstream,
for the adequacy gate (`Adequacy.lean`, Phase 3) which quantifies over the resulting
consequence relation.

**Task 517 scope note**: this file (and `Deduction.lean`) covers Phases 1-2 of
`plans/01_labelled-framework.md` only. Phases 3+ (adequacy, contexts, the Prime Lemma, the
canonical model, the birelation construction, frame-class match, and final assembly of
`cs5_completeness`) are dispatched separately and are **not** attempted here.
...
- `Coinfinite`: a subset `V' ⊆ V` is coinfinite iff its complement is infinite. The fresh-label
  ...
  Canonical Model Lemma 5.3.2 (Phase 6, not attempted here, but the supply lemma is a Phase 1
  deliverable per the plan).
...
- `Graph Atom`: a non-empty set of nodes `X` with an edge relation `R` confined to `X`, plus the
  ...
  `Graph.le` is the "sub-graph" order used by the weakening lemmas (Phase 2) and the canonical
  model's `≤` (Phase 6, not attempted here).
- `LabelledFormula Atom`: Simpson's `x : A` judgement, as a structure with fields `lbl` and
  `prop`.

## Definitional choice (flagged for the Phase 3/4/5/6 implementers)
...
This module implements only the diamond-witness operator `dwitness`. The task's target frame
theory is `𝒯_S5 := {χ_T, χ_5}` (fixed in Phase 4): both `χ_T` (reflexivity, `∀x, xRx`) and `χ_5`
(Euclideanness, `∀x y z, xRy → xRz → yRz`) are **universal Horn sentences with no existential in
the conclusion**, so neither needs a Skolem/geometric-sequent witness operator. The omission is
therefore harmless for this task's scope. Extending `Label` with additional witness
constructors later (e.g. for `χ_D`) is purely additive (a new constructor) and does not disturb
anything defined here or in `Deduction.lean`.
...
by the weakening/graph-morphism lemmas (Phase 2) and, downstream, the canonical model's `≤`
(Phase 6, not attempted here). -/
