...
instance :
    HasAxiomAndI Modal.HilbertK45
      (F := Modal.Proposition Atom) where
  andI := ⟨Modal.DerivationTree.ax [] _ (Modal.K45Axiom.andI _ _)⟩

instance :
    HasAxiomAndE1 Modal.HilbertK45
      (F := Modal.Proposition Atom) where
  andE1 := ⟨Modal.DerivationTree.ax [] _ (Modal.K45Axiom.andE1 _ _)⟩

instance :
    HasAxiomAndE2 Modal.HilbertK45
      (F := Modal.Proposition Atom) where
  andE2 := ⟨Modal.DerivationTree.ax [] _ (Modal.K45Axiom.andE2 _ _)⟩

instance :
    HasAxiomOrI1 Modal.HilbertK45
      (F := Modal.Proposition Atom) where
  orI1 := ⟨Modal.DerivationTree.ax [] _ (Modal.K45Axiom.orI1 _ _)⟩

instance :
    HasAxiomOrI2 Modal.HilbertK45
      (F := Modal.Proposition Atom) where
  orI2 := ⟨Modal.DerivationTree.ax [] _ (Modal.K45Axiom.orI2 _ _)⟩

instance :
    HasAxiomOrE Modal.HilbertK45
      (F := Modal.Proposition Atom) where
  orE := ⟨Modal.DerivationTree.ax [] _ (Modal.K45Axiom.orE _ _ _)⟩
