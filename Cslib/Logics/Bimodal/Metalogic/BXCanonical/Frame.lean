...
lemma hContentClosedDerivation {Omega : Set (Formula Atom)} {φ : Formula Atom}
  ...
  have d_H : DerivationTree FrameClass.Base (Context.map Formula.allPast L) (Formula.allPast φ) :=
    Theorems.generalizedPastK L φ h_deriv
...
theorem g_content_set_consistent {Omega : Set (Formula Atom)}
  ...
  have h_serial_in := theoremInMcsFc h_mcs h_serial
  ...
  have h_top_in := theoremInMcsFc h_mcs h_top
...
theorem bx_le_trans {w u v : BXPoint Atom} (hwu : bxLe w u) (huv : bxLe u v) :
    bxLe w v := by
  intro φ hφ
...
lemma bxGBackward (w : BXPoint Atom) (φ : Formula Atom)
  ...
  have h_seed_cons : SetConsistent FrameClass.Base
    ...
    · let L_filt := L.filter (fun y => decide (y ≠ Formula.neg φ))
      ...
      have h_Gφ := gContentClosedDerivation w.is_mcs L_filt h_filt_in_g d_phi
      exact h_not_G h_Gφ
...
lemma bxHBackward (w : BXPoint Atom) (φ : Formula Atom)
  ...
  have h_seed_cons : SetConsistent FrameClass.Base
    ...
    · let L_filt := L.filter (fun y => decide (y ≠ Formula.neg φ))
      ...
      have d_phi : DerivationTree FrameClass.Base L_filt φ :=
        DerivationTree.modus_ponens L_filt _ _ d_dne_weak d_negneg
      have h_Hφ := hContentClosedDerivation w.is_mcs L_filt h_filt_in_h d_phi
      exact h_not_H h_Hφ
...
theorem bx_modal_equiv_refl (w : BXPoint Atom) : bxModalEquiv w w :=
  fun _ => Iff.rfl

theorem bx_modal_equiv_symm {w v : BXPoint Atom} (h : bxModalEquiv w v) :
    bxModalEquiv v w :=
  fun φ => (h φ).symm
  ...
  have h_seed_cons : SetConsistent FrameClass.Base ({ψ} ∪ bc : Set (Formula Atom)) := by
    ...
    · let L_filt := L.filter (fun y => decide (y ≠ ψ))
      ...
      have d_neg : DerivationTree FrameClass.Base L_filt (Formula.neg ψ) :=
        deductionTheorem L_filt ψ (Formula.bot : Formula Atom) d_reord
...
theorem box_preserved_along_bx_le {w v : BXPoint Atom} (h_le : bxLe w v) (φ : Formula Atom) :
  ...
  · intro h_box_v
    ...
    have h_box_neg := SetMaximalConsistent.implication_property w.is_mcs
      (theoremInMcsFc w.is_mcs (negBoxToBoxNegBox' φ)) h_neg_box
    have h_tf2 : DerivationTree FrameClass.Base [] ((Formula.box (Formula.box φ).neg).imp
      ...
      Theorems.Combinators.tempFutureDerived (Formula.box φ).neg
...
theorem bx_modal_equiv_of_bx_le {w v : BXPoint Atom} (h_le : bxLe w v) :
    bxModalEquiv w v :=
