...
- [ ] Prove `modalStepBranchS5g_preserves_keysTotal` — every newly-known world gets a recorded key; consumes `accKnown` (P3) feeding `modalApplyOne_knownWorlds_step` (`FmpMeasure.lean:2042`) for the label-provenance dichotomy. *(deviation: attempted cycle 5, NOT LANDED -- reverted before commit, see below. Recommended to reuse `modalApplyOneS5_knownWorlds_step` (landed Phase 4, cycle 4) + `accKnown` directly -- see handoff `03_phase5-keysdistinct-landed-keylowerbd-scope.md` and the cycle-5 handoff for the exact obstruction and two small reusable K/S5-level lemmas that DID compile standalone.)*
