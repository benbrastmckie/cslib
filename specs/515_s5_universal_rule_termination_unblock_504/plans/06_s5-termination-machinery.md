# Implementation Plan: S5 Witness-Reuse Rule + Linear World Budget + Euclidean 5/KB5 Route (v5)

- **Task**: 515 - s5_universal_rule_termination_unblock_504
- **Status**: [IMPLEMENTING]
- **Effort**: 52 hours (24 phases: 27h S5 chain [Phases 0-14] + 25h Euclidean 5/KB5 chain [Phases 15-23]; the figure is the exact sum of the per-phase timings and is deliberately pessimistic -- see "Budget realism" below). **v5 does not change the total**: the Phase 19 remediation (Route 1) is *bounded* -- it revises two existing helpers, keeps every mint-arm proof, and makes the world-bound spec fields strictly easier; the report (`reports/07_*`) shows it fits inside Phase 19's existing 3h/400-line budget with no cascade into termination or the K/T/B/S5 surface.
...
- **Research Inputs**:
  ...
  - reports/07_phase19-soundness-blocker-remediation.md (**new in v5**; the authority for the Phase 19 remediation. Verdict: adopt **Route (1)**, root/non-root asymmetric propagation. Root cause: `fiveFC := RightEuclidean r` has no reflexivity conjunct, so `reachable_imp_related_s5`'s `hFC.1.refl` base case does not port; machine-checked `Fin 3` counterexample confirms uniform trigger-independent propagation is unsound when triggered from the root. Reuses the landed Phase 17 cod-equivalence lemmas.)
  ...
  - plans/05_s5-termination-machinery.md (v4; superseded by this file -- carried the Phase 19 `[BLOCKED]` record now resolved here)
  - plans/04_s5-termination-machinery.md (v3; superseded by v4)
- **Artifacts**: plans/06_s5-termination-machinery.md (this file)
- **Standards**:
...
the blocker-remediation research that resolves the Phase 19 `[BLOCKED]` record carried by v4. **The
decision is made and is NOT re-litigated here**: adopt **Route (1)**, root/non-root asymmetric
propagation. This revision bakes that corrected design into Phases 19-23 (which move from `[BLOCKED]`
to `[NOT STARTED]`) and updates Phase 18's post-hoc caveat to point at the resolution. Finding 5
below records the substance; the diagnosis it resolves is **preserved** (not deleted) in Phase 19's
"Resolved blocker record" so a future dispatch sees both the gap and its fix. It changes **nothing**
about the S5 architecture (Phases 0-14) and **nothing** about Phases 15-18's landed assets.
...
This v5 supersedes `plans/05_s5-termination-machinery.md` (v4), which superseded
...
**POST-HOC CAVEAT -- RESOLVED in v5 (see `reports/07_*` and Phase 19 below)**: the
...
scaffolding) independent of frame semantics, and this phase remains `[COMPLETED]`. The propagation
...
from scratch). That refinement + the re-verification is executed at the **head of Phase 19**, where the
soundness proof consumes it -- so this phase is not demoted and nothing it committed is discarded. **Do
not treat the *original* uniform propagation shape as final** -- Phase 19 installs the root/non-root
...
cod-equivalence discharge. Mirrors Phase 13, at the Euclidean frame class, with the root/non-root
split the Euclidean class requires.

**DECISION (made in `reports/07_*`; DO NOT re-litigate)**: Adopt **Route (1)** -- root/non-root
asymmetric propagation. The full remediation spec is in
`reports/07_phase19-soundness-blocker-remediation.md`. Route (2) (completeness cross-check)
**converges** with Route (1) rather than competing (see Phase 20's `EuclGen` note); Route (c) /
Fallback 4 (S5-only) is **not** warranted -- the fix is bounded.

**RESOLVED BLOCKER RECORD (diagnosis preserved for provenance; the fix is below)**: v4 recorded a
`[BLOCKED]` here, hit at the design-analysis stage before any Lean proof was attempted. The diagnosis
is retained verbatim so a future dispatch sees both the gap and why the fix closes it -- it is **not**
deleted. The remediation (Route 1) follows in **Tasks**.
...
**Timing**: 3 hours (helper revision + re-verification + soundness; the report shows Route 1 fits this
budget with no cascade)

**Depends on**: 18

**Files to modify**: `Cslib/Logics/Modal/Tableau/FiveSimplification.lean` (Route 1 helper revision +
`modalApplyOneFive_specCore` re-verification), `Cslib/Logics/Modal/Tableau/FrameSoundness.lean`
(`accReachableInv_related_five` + `modalTableauFive_sound`). **`GenericDriver.lean` NOT touched.**

**Verification**: full CI green; `lean_verify` on `modalTableauFive_sound` (expect
`propext`/`Classical.choice`/`Quot.sound` only); `modalApplyOneFive_specCore` still typechecks under
the revised helpers; **zero edits to any K/T/B/S4/S5 declaration's statement**; the S5 surface
(`modalTableauS5*`, `extractModelS5*`, `modalTruthLemmaS5`, `instDecidableS5Valid`) untouched.

**Blocked-branch**: if Route 1's soundness proof genuinely resists within the ~400-line budget,
`[BLOCKED]` with the exact `lean_goal` open state + the measured line count, then invoke **Fallback 4**
(land Phases 0-14 as the S5 deliverable; 5/KB5 chain resumes via a follow-up carrying the open goal --
**never** re-narrated as impossible). No `sorry`.

---

### Phase 20: `extractModelFive` + the Euclidean truth lemma [NOT STARTED]

**Unblocked in v5** -- Phase 19 is resolved via Route (1). **Route (1)'s root/non-root split is
consistent with -- indeed anticipated by -- this phase's `EuclGen` structure**: `EuclGen`'s `eucl`
constructor only combines two derivations **sharing the same first argument**, so
...
`EuclGen r 1 2`. That is exactly the asymmetric shape Route 1 installs for soundness, so the same
shape serves **both** halves (the strong consistency signal recorded in `reports/07_*`, "Convergence
with completeness").
...
**Unblocked in v5** -- consumes Phase 19's (Route-1) soundness and Phase 20's countermodel.
...
**Unblocked in v5** -- KB5 reuses the **Route-1-corrected** Five pattern (the same root/non-root
propagation split). The KB5 PER normal form (symmetric + Euclidean ⇒ transitive) makes the root/cluster
dichotomy even cleaner; the mint arms remain shape-identical to `modalApplyOneS5w`'s.
...
**Unblocked in v5** -- the capstone; consumes Phases 19-22.
...
- **P19**: Route-1 helper revision (`modalFiveBoxAll`/`modalFiveDiaNegAll` + `acc` param + root-arm
  `hasEdge 0 w'` guard; re-verified `modalApplyOneFive_specCore`) (`FiveSimplification.lean`);
  `accReachableInv_related_five`, `modalTableauFive_sound` (`FrameSoundness.lean`). `GenericDriver.lean`
  untouched.
