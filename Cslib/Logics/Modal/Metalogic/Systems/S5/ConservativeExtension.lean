...
/-! # Modal S5 as a Conservative Extension of Classical Propositional Logic

This module proves that Modal S5 is a conservative extension of Classical Propositional Logic
(CPL): any propositional formula `φ` that is derivable in S5 (as `φ.toModal`) is already
derivable in CPL.

The proof uses the universal model `(Unit, fun _ _ => True, fun _ => v)`, which satisfies
all S5 frame conditions (reflexivity: `fun _ => trivial`, transitivity:
`fun _ _ _ _ _ => trivial`, and Euclideanness: `fun _ _ _ _ _ => trivial`). Since `φ.toModal`
...
- `s5_conservative_extension`: If `φ.toModal` is S5-derivable then `φ` is CPL-derivable.
...
/-- **Modal S5 Conservative Extension over CPL**:

If the modal translation `φ.toModal` is derivable in S5, then `φ` is derivable
in Classical Propositional Logic.

Proof: construct the universal model `(Unit, fun _ _ => True, fun _ => v)`.
Reflexivity, transitivity, and Euclideanness all hold vacuously. S5 soundness gives
satisfaction of `φ.toModal` at the single world; the bridge lemma converts this to
`Evaluate v φ`; CPL completeness then gives CPL derivability. -/
theorem s5_conservative_extension {Atom : Type*} {φ : PL.Proposition Atom}
  ...
  exact (modal_satisfies_toModal_iff_evaluate m () φ).mp
    (s5_soundness d m (fun _ => trivial) (fun _ _ _ _ _ => trivial)
